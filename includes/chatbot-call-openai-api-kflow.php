<?php
/**
 * Steven-Bot - Kflow API - Ver 1.9.5
 *
...
function steven_bot_call_flow_api($api_key, $message, $user_id = null, $page_id = null, $session_id = null, $assistant_id = null, $client_message_id = null) {
    ...
    $kflow_sequence = get_steven_bot_transients('kflow_sequence', null, null, $session_id);
    $kflow_step = (int) get_steven_bot_transients('kflow_step', null, null, $session_id);
    ...
    if ($kflow_step > $max_answers) {
        ...
        $answers = [];
        $answers = steven_bot_retrieve_answers($session_id, $user_id, $page_id, $assistant_id, $max_answers);

        // Parse the template inserting the answers
        $message = steven_bot_parse_template($template, $answers);
        ...
        $message = steven_bot_custom_gpt_call_api($api_key, $message, $assistant_id, $thread_id, $session_id, $user_id, $page_id);
        ...
        set_steven_bot_transients('kflow_sequence', $kflow_sequence, null, null, $session_id);
...
function steven_bot_retrieve_answers($session_id, $user_id, $page_id, $assistant_id, $max_answers) {
...
function steven_bot_parse_template($template, $answers) {
    ...
    if (empty($template)) {
        // DIAG - Diagnostics
        // back_trace( 'NOTICE', 'steven_bot_parse_template() - $template is empty');
        return $message;
    }
