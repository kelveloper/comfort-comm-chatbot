<?php
/**
 * Steven-Bot - [chatbot_chatgpt] Shortcode Registration
 *
...
function steven_bot_shortcode( $atts = [], $content = null, $tag = '' ) {
    ...
    // For anonymous users, user_id should be 0 (session_id is used in transient key via set_steven_bot_transients)
    ...
    if (esc_attr(get_option('chatbot_nvidia_api_enabled', 'No')) == 'Yes') {
        ...
        $kchat_settings['steven_bot_model'] = $model_choice;
        ...
        $kchat_settings['steven_bot_model'] = $model_choice;
        $voice_choice = esc_attr(get_option('steven_bot_voice_option', 'alloy'));    
    }
    ...
    if (strpos($tag, 'chatbot-') !== false) {
        ...
        $assistant_details = get_steven_bot_assistant_by_key($assistant_key);
    ...
    if ( !empty($atts['assistant']) && strpos($atts['assistant'], 'asst_') === false && strpos($atts['assistant'], 'ag:') === false && strpos($atts['assistant'], 'websearch') === false) {
        ...
        // Utility
        $assistant_details = get_steven_bot_assistant_by_common_name($assistantCommonName);
        ...
        if (empty($assistant_details)) {
            ...
            // Set to original
            $steven_bot_assistant_alias = 'original'; // default value
            ...
            $steven_bot_assistant_alias = $assistant_details['assistant_id'];
        ...
        // Set the assistant_id
        $steven_bot_assistant_alias = $atts['assistant'];
    ...
    $steven_bot_display_style = 'floating'; // default value
    ...
    if (array_key_exists('style', $atts) && !is_null($atts['style'])) {
        ...
        if (in_array($atts['style'], $valid_styles)) {
            ...
            $steven_bot_display_style = sanitize_text_field($atts['style']);
            $kchat_settings['steven_bot_display_style'] = $steven_bot_display_style;
            // back_trace( 'NOTICE', '$steven_bot_display_style: ' . $steven_bot_display_style);
        } else {
            ...
            // back_trace( 'ERROR', 'Invalid display style: ' . $steven_bot_display_style);
        }
    ...
    // $temp_steven_bot_display_style = get_steven_bot_transients('display_style', $user_id, $page_id, $session_id, null);

    // One bot per page, embedded over floating - Ver 2.1.6
    // if ($kchat_settings['steven_bot_display_style'] == 'floating' && $temp_steven_bot_display_style == 'embedded') {
    //     // back_trace( 'NOTICE', 'Embedded style selected over floating style');
    //     // End the shortcode processing
    //     // return;
    //     // $atts['style'] = 'embedded';
    //     $steven_bot_display_style = 'embedded';
    //     $kchat_settings['steven_bot_display_style'] = 'embedded';
    // }

    // Set the sanitized display style in $atts and $kchat_settings
    $atts['steven_bot_display_style'] = $steven_bot_display_style;
    $kchat_settings['steven_bot_display_style'] = $steven_bot_display_style;
    // back_trace( 'NOTICE', '$steven_bot_display_style: ' . $steven_bot_display_style);
    ...
    if (array_key_exists('assistant', $atts)) {
        ...
        if (in_array($sanitized_assistant, $valid_ids) || strpos($sanitized_assistant, 'asst_') === 0 || strpos($sanitized_assistant, 'ag:') === 0 ||strpos($sanitized_assistant, 'websearch') === 0 ) {
            $steven_bot_assistant_alias = $sanitized_assistant;
            // back_trace( 'NOTICE', '$assistant_id: ' . $steven_bot_assistant_alias);
        } else {
    ...
    $steven_bot_audience_choice_global = esc_attr(get_option('steven_bot_audience_choice', 'all'));
    $steven_bot_audience_choice = $steven_bot_audience_choice_global; // default value
    if (array_key_exists('audience', $atts)) {
        ...
        if (in_array($sanitized_audience, $valid_audiences)) {
            $steven_bot_audience_choice = $sanitized_audience;
            $kchat_settings['steven_bot_audience_choice'] = $steven_bot_audience_choice;
            // back_trace( 'NOTICE', '$steven_bot_audience_choice: ' . $steven_bot_audience_choice);
        } else {
            $steven_bot_audience_choice = $steven_bot_audience_choice_global;
            $atts['audience'] = $steven_bot_audience_choice_global;
            $kchat_settings['steven_bot_audience_choice'] = $steven_bot_audience_choice_global;
            // back_trace( 'ERROR', 'Invalid audience choice: ' . $sanitized_audience);
        }
    }
    
    // Validate and sanitize the prompt parameter - Ver 1.9.9
    $steven_bot_hot_bot_prompt = ''; // default value
    if (array_key_exists('prompt', $atts)) {
        $steven_bot_hot_bot_prompt = sanitize_text_field($atts['prompt']);
        // back_trace( 'NOTICE', 'steven_bot_hot_bot_prompt: ' . $steven_bot_hot_bot_prompt);
    } elseif (isset($_GET['chatbot_prompt'])) {
        $steven_bot_hot_bot_prompt = sanitize_text_field($_GET['chatbot_prompt']);
        // back_trace( 'NOTICE', 'steven_bot_hot_bot_prompt: ' . $steven_bot_hot_bot_prompt);
    }
    If (!empty($steven_bot_hot_bot_prompt)) {
        $steven_bot_hot_bot_prompt = preg_replace("/^\\\\'|\\\\'$/", '', $steven_bot_hot_bot_prompt);
    }
    ...
    if (array_key_exists('additional_instructions', $atts)) {
        ...
        $kchat_settings['steven_bot_additional_instructions'] = $additional_instructions;
    ...
    if (!isset($atts['model'])) {
        ...
        $kchat_settings['steven_bot_model_choice'] = $model;
        $assistant_details['model'] = $model;
        $assistant_details['steven_bot_model_choice'] = $model;
        $kchat_settings['model'] = $model;
        $kchat_settings['steven_bot_model_choice'] = $model;
    ...
    if (array_key_exists('voice', $atts)) {
        ...
        if (in_array($sanitized_voice, $valid_voices)) {
            ...
            $kchat_settings['steven_bot_voice_option'] = $voice;
            $assistant_details['voice'] = $voice;
            $assistant_details['steven_bot_voice_option'] = $voice;
            $kchat_settings['voice'] = $voice;
            $kchat_settings['steven_bot_voice_option'] = $voice;
            // back_trace( 'NOTICE', '$voice: ' . $voice);
        } else {
            $voice = esc_attr(get_option('steven_bot_voice_option', 'alloy'));
            $kchat_settings['voice'] = $voice;
            $kchat_settings['steven_bot_voice_option'] = $voice;
            $assistant_details['voice'] = $voice;
            $assistant_details['steven_bot_voice_option'] = $voice;
    ...
    If ($user_logged_in) {
        if ($steven_bot_audience_choice == 'all' || $steven_bot_audience_choice == 'logged-in') {
            // Ok to proceed
        } else {
            // Not ok to proceed
        ...
        if ($steven_bot_audience_choice == 'all' || $steven_bot_audience_choice == 'visitors') {
            // Ok to proceed
        } else {
            // Not ok to proceed
    ...
    if ( $steven_bot_assistant_alias == 'primary' ) {
        $assistant_id = esc_attr(get_option('assistant_id', ''));
        $additional_instructions = esc_attr(get_option('steven_bot_additional_instructions', ''));
    } elseif ( $steven_bot_assistant_alias == 'alternate' ) {
        $assistant_id = esc_attr(get_option('steven_bot_assistant_id_alternate', ''));
        $additional_instructions = esc_attr(get_option('steven_bot_additional_instructions_alternate', ''));
    } else {
    ...
    // back_trace( 'NOTICE', 'get_steven_bot_transients: ' . get_steven_bot_transients('display_style', $user_id, $page_id, $session_id));
    // if (get_steven_bot_transients('display_style', $user_id, $page_id, $session_id) == 'embedded') {
    //     // back_trace( 'NOTICE', 'Embedded chatbot detected');
    //     $steven_bot_display_style = 'embedded';
    //     return;
    // }

    set_steven_bot_transients( 'display_style' , $steven_bot_display_style, $user_id, $page_id, $session_id, null );
    set_steven_bot_transients( 'assistant_alias' , $steven_bot_assistant_alias, $user_id, $page_id, $session_id, null );
    
    set_steven_bot_transients( 'assistant_id', $assistant_id, $user_id, $page_id, $session_id, null);
    set_steven_bot_transients( 'thread_id', $thread_id, $user_id, $page_id, $session_id, null);

    set_steven_bot_transients( 'additional_instructions', $additional_instructions, $user_id, $page_id, $session_id, null);
    ...
    $kchat_settings = array_merge($kchat_settings, array(
        ...
        'steven_bot_appearance_collapse_icon' => esc_attr(get_option('steven_bot_appearance_collapse_icon', '')),
        'steven_bot_appearance_erase_icon' => esc_attr(get_option('steven_bot_appearance_erase_icon', '')),
        'steven_bot_appearance_mic_enabled_icon' => esc_attr(get_option('steven_bot_appearance_mic_enabled_icon', '')),
        'steven_bot_appearance_mic_disabled_icon' => esc_attr(get_option('steven_bot_appearance_mic_disabled_icon', '')),
    ));
    ...
    if ($chatbot_ai_platform_choice == 'OpenAI'){
        $use_assistant_name = esc_attr(get_option('steven_bot_display_custom_gpt_assistant_name', 'Yes'));
    ...
    if ($use_assistant_name == 'Yes' && !empty($assistant_id) && $assistant_id !== 'original') {
        ...
        // back_trace( 'NOTICE', '$assistant_id: ' . $assistant_id);
        // back_trace( 'NOTICE', '$use_assistant_name: ' . $use_assistant_name);
        $assistant_name = esc_attr(get_steven_bot_assistant_name($assistant_id));
        // back_trace( 'NOTICE', '$assistant_name: ' . $assistant_name);
        $bot_name = !empty($assistant_name) ? $assistant_name : esc_attr(get_option('steven_bot_bot_name', 'Steven-Bot'));
    } else {
        $bot_name = esc_attr(get_option('steven_bot_bot_name', 'Steven-Bot'));
    }
    ...
    // wp_add_inline_script('steven-bot-local-js', 'if (typeof kchat_settings === "undefined") { var kchat_settings = ' . $escaped_kchat_settings_json . '; } else { kchat_settings = ' . $escaped_kchat_settings_json . '; }', 'before');
    // wp_add_inline_script('steven-bot-js', 'if (typeof kchat_settings === "undefined") { var kchat_settings = ' . $escaped_kchat_settings_json . '; } else { kchat_settings = ' . $escaped_kchat_settings_json . '; }', 'before');

    $steven_bot_bot_prompt = esc_attr(get_option('steven_bot_bot_prompt', 'Enter your question ...'));
    ...
    if (!empty($steven_bot_hot_bot_prompt)) {
        // back_trace( 'NOTICE', 'Hot Prompting the Chatbot');
            wp_add_inline_script('steven-bot-js', '
            if (typeof kchat_settings === "undefined") { 
                var kchat_settings = ' . $kchat_settings_json . '; 
            } else { 
                kchat_settings = ' . $kchat_settings_json . '; 
            }
            document.getElementById("steven-bot-message").placeholder = "' . $steven_bot_hot_bot_prompt . '";
        ', 'before');
    }
    ...
    $steven_bot_allow_mp3_uploads = 'No';
    ...
    if (strpos($steven_bot_assistant_alias,'asst_') !== false) {
        ...
        if ( !empty($assistant_details['allow_file_uploads']) ) {
            $steven_bot_allow_file_uploads = $assistant_details['allow_file_uploads'];
        }
    ...
    if (strpos($model, 'whisper') !== false) {
        $steven_bot_allow_file_uploads = 'No';
        $steven_bot_allow_mp3_uploads = 'Yes';
    }

    if (strpos($model, 'gpt-4o') !== false && strpos($steven_bot_assistant_alias, 'asst_') === false && $steven_bot_assistant_alias !== 'original') {
        // $steven_bot_allow_file_uploads = esc_attr(get_option('steven_bot_allow_file_uploads', 'No'));
        $steven_bot_allow_file_uploads = 'No';
        $steven_bot_allow_mp3_uploads = 'No';
    }
    ...
    // $steven_bot_allow_file_uploads = 'Yes';
    ...
    $steven_bot_allow_download_transcript = esc_attr(get_option('steven_bot_allow_download_transcript', 'Yes'));
    // Check if the key exists and is not empty, otherwise use default
    $steven_bot_allow_download_transcript = isset($assistant_details['allow_transcript_downloads']) && !empty($assistant_details['allow_transcript_downloads']) ? $assistant_details['allow_transcript_downloads'] : $steven_bot_allow_download_transcript;
    ...
    if (function_exists('kflow_prompt_and_response') && !empty($atts['sequence']) && $kflow_enabled) {
        ...
        // Set transients
        set_steven_bot_transients('kflow_sequence', $sequence_id, null, null, $session_id);
        set_steven_bot_transients('kflow_step', 0, null, null, $session_id);
        ...
        if ( $kflow_prompt != '' ) {
            ...
            // Set transients
            set_steven_bot_transients('kflow_sequence', $sequence_id, null, null, $session_id);
            set_steven_bot_transients('kflow_step', -1, null, null, $session_id); // Start at -1 not 0
            ...
            $steven_bot_hot_bot_prompt = '[Chatbot]' . $kflow_prompt;
    ...
    $steven_bot_width_setting = esc_attr(get_option('steven_bot_width_setting', 'Narrow'));
    ...
    $steven_bot_start_status_new_visitor = esc_attr(get_option('steven_bot_start_status_new_visitor', 'closed'));
    // $assistant_details['steven_bot_start_status_new_visitor'] = $steven_bot_start_status_new_visitor;
    $kchat_settings['steven_bot_start_status_new_visitor'] = $steven_bot_start_status_new_visitor;
    ...
    $assistant_details['steven_bot_initial_greeting'] = $assistant_details['initial_greeting'];
    $kchat_settings['steven_bot_initial_greeting'] = $assistant_details['initial_greeting'];

    $assistant_details['steven_bot_subsequent_greeting'] = $assistant_details['subsequent_greeting'];
    $kchat_settings['steven_bot_subsequent_greeting'] = $assistant_details['subsequent_greeting'];
    ...
    // back_trace( 'ERROR', '$kchat_settings[\'steven_bot_display_style\']: ' . $kchat_settings['steven_bot_display_style']);
    // back_trace( 'ERROR', '$assistant_details[\'audience\']: ' . $assistant_details['audience']);
    // back_trace( 'ERROR', '$kchat_settings[\'steven_bot_audience_choice\']: ' . $kchat_settings['steven_bot_audience_choice']);

    // Last chance to set localStorage - Ver 2.0.5
    // back_trace( 'NOTICE', 'BEFORE: $assistant_details[\'style\']: ' . $assistant_details['style']);
    // $assistant_details['style'] = !empty($assistant_details['style']) ? $assistant_details['style'] : esc_attr(get_option('steven_bot_display_style', 'floating'));
    // back_trace( 'NOTICE', 'AFTER: $assistant_details[\'style\']: ' . $assistant_details['style']);
    // $kchat_settings['steven_bot_display_style'] = $assistant_details['style'];

    $assistant_details['audience'] = !empty($assistant_details['audience']) ? $assistant_details['audience'] : esc_attr(get_option('steven_bot_audience_choice', 'All'));
    $kchat_settings['steven_bot_audience_choice'] = $assistant_details['audience'];
    
    // DIAG - Diagnostics - Ver 2.0.5
    // back_trace( 'NOTICE', 'BEFORE: $assistant_details[\'voice\']: ' . $assistant_details['voice']);
    // back_trace( 'NOTICE', 'BEFORE: $kchat_settings[\'steven_bot_voice_option\']: ' . $kchat_settings['steven_bot_voice_option']);
    // back_trace( 'NOTICE', 'BEFORE: $kchat_settings[\'voice\']: ' . $kchat_settings['voice']);
    
    $assistant_details['voice'] = !empty($assistant_details['voice']) ? $assistant_details['voice'] : esc_attr(get_option('steven_bot_voice_option', 'alloy'));
    $kchat_settings['steven_bot_voice_option'] = $assistant_details['voice'];
    $kchat_settings['voice'] = $assistant_details['voice'];
    set_steven_bot_transients('voice', $assistant_details['voice'], $user_id, $page_id, $session_id, null);

    // DIAG - Diagnostics - Ver 2.0.5
    // back_trace( 'NOTICE', 'AFTER: $assistant_details[\'voice\']: ' . $assistant_details['voice']);
    // back_trace( 'NOTICE', 'AFTER: $kchat_settings[\'steven_bot_voice_option\']: ' . $kchat_settings['steven_bot_voice_option']);
    // back_trace( 'NOTICE', 'AFTER: $kchat_settings[\'voice\']: ' . $kchat_settings['voice']);

    $assistant_details['allow_file_uploads'] = !empty($assistant_details['allow_file_uploads']) ? $assistant_details['allow_file_uploads'] : esc_attr(get_option('steven_bot_allow_file_uploads', 'No'));
    $kchat_settings['steven_bot_allow_file_uploads'] = $assistant_details['allow_file_uploads'];
    // DIAG - Diagnostics - Ver 2.2.6
    ...
    $kchat_settings['steven_bot_allow_download_transcript'] = $assistant_details['allow_transcript_downloads'];

    $assistant_details['additional_instructions'] = !empty($assistant_details['additional_instructions']) ? $assistant_details['additional_instructions'] : esc_attr(get_option('steven_bot_additional_instructions', ''));
    $kchat_settings['steven_bot_additional_instructions'] = $assistant_details['additional_instructions'];

    $assistant_details['force_page_reload'] = !empty($assistant_details['force_page_reload']) ? $assistant_details['force_page_reload'] : esc_attr(get_option('steven_bot_force_page_reload', 'Yes'));
    $kchat_settings['steven_bot_force_page_reload'] = $assistant_details['force_page_reload'];

    $assistant_details['conversation_continuation'] = !empty($assistant_details['conversation_continuation']) ? $assistant_details['conversation_continuation'] : esc_attr(get_option('steven_bot_conversation_continuation', 'On'));
    $kchat_settings['steven_bot_conversation_continuation'] = $assistant_details['conversation_continuation'];

    $assistant_details['width'] = !empty($assistant_details['width']) ? $assistant_details['width'] : esc_attr(get_option('steven_bot_width_setting', '300'));
    $kchat_settings['steven_bot_width_setting'] = $assistant_details['width'];

    $assistant_details['common_name'] = !empty($assistant_details['common_name']) ? $assistant_details['common_name'] : esc_attr(get_option('steven_bot_bot_name', 'Steven-Bot'));
    $kchat_settings['steven_bot_bot_name'] = $assistant_details['common_name'];
    $kchat_settings['steven_bot_bot_name'] = !empty($assistant_details['common_name']) ? $assistant_details['common_name'] : esc_attr(get_option('steven_bot_bot_name', 'Steven-Bot'));
    ...
    $kchat_settings['steven_bot_version'] = $steven_bot_plugin_version;
    $kchat_settings_json = wp_json_encode($kchat_settings);
    wp_add_inline_script('steven-bot-local-js', 'if (typeof kchat_settings === "undefined") { var kchat_settings = ' . $kchat_settings_json . '; } else { kchat_settings = ' . $kchat_settings_json . '; }', 'before');
    ...
    $kchat_settings['steven_bot_avatar_icon_setting'] = esc_attr(get_option('steven_bot_avatar_icon_setting', ''));
    $kchat_settings['steven_bot_custom_avatar_icon_setting'] = esc_attr(get_option('steven_bot_custom_avatar_icon_setting', ''));
    ...
    if ($steven_bot_display_style == 'embedded') {
        // Code for embed style ('embedded' is the alternative style)
        // Store the style and the assistant value - Ver 1.7.2
        set_steven_bot_transients( 'display_style' , $steven_bot_display_style, $user_id, $page_id, $session_id, null );
        set_steven_bot_transients( 'assistant_alias' , $steven_bot_assistant_alias, $user_id, $page_id, $session_id, null );
        set_steven_bot_transients( 'model' , $model, $user_id, $page_id, $session_id, null);
        set_steven_bot_transients( 'voice' , $voice, $user_id, $page_id, $session_id, null);
        set_steven_bot_transients( 'assistant_name' , $bot_name, $user_id, $page_id, $session_id, null);
        // OUTSIDE THE IF STATEMENT - Ver 2.0.5 - 2024 07 05
        // ob_start();
        ?>
        <div id="steven-bot" style="display: flex;" class="chatbot-embedded-style chatbot-full" data-cache-buster="<?php echo time(); ?>">
        <script>
            jQuery(document).ready(function($) {
                $('#steven-bot').removeClass('chatbot-floating-style').addClass('chatbot-embedded-style');
            });
        </script>
        <!-- REMOVED FOR EMBEDDED -->
        <?php
        // if ( $use_assistant_name == 'Yes' ) {
        //     echo '<div id="steven-bot-header-embedded">';
        //     echo '<div id="steven-bot-title" class="title">' . strip_tags($bot_name) . '</div>';
        //     echo '</div>';
        // } else {
            echo '<div id="steven-bot-header-embedded">';
            echo '<div id="steven-bot-title" class="title"></div>';
            echo '</div>';
        // }
        ?>
        <div id="steven-bot-conversation"></div>
        <div id="steven-bot-input">
            <div id="steven-bot-input-area">
                <label for="steven-bot-message"></label>
                <?php
                    ...
                    if ($kflow_enabled == true and !empty($sequence_id)) {
                        ...
                        echo "<input type='hidden' id='steven-bot-message' value='" . htmlspecialchars($steven_bot_hot_bot_prompt, ENT_QUOTES) . "'>";
                        // echo "<textarea id='steven-bot-message' rows='2' placeholder='$steven_bot_bot_prompt' style='width: 95%;'></textarea>";
                        echo "<script>
                        document.addEventListener('DOMContentLoaded', function() {
                            var hiddenInput = document.getElementById('steven-bot-message');
                            var submitButton = document.getElementById('steven-bot-submit');
                            if (submitButton) {
                    ...
                    if ($kflow_enabled != true and !empty($steven_bot_hot_bot_prompt)) {
                        // DIAG - Diagnostics - Ver 1.9.0
                        // back_trace( 'NOTICE', 'PRELOAD: $steven_bot_hot_bot_prompt: ' . $steven_bot_hot_bot_prompt);
                        $rows = esc_attr(get_option('steven_bot_input_rows', '2'));
                        $steven_bot_bot_prompt = esc_attr(sanitize_text_field($steven_bot_bot_prompt));
                        $steven_bot_hot_bot_prompt = esc_attr(sanitize_text_field($steven_bot_hot_bot_prompt));
                        // Ver 2.5.0: Removed inline width style - CSS handles alignment
                        echo "<textarea id='steven-bot-message' rows='". htmlspecialchars($rows) . "' placeholder='" . htmlspecialchars($steven_bot_bot_prompt) . "'>" . $steven_bot_hot_bot_prompt . "</textarea>";
                        echo "<script>
                        document.addEventListener('DOMContentLoaded', function() {
                            var textarea = document.getElementById('steven-bot-message');
                            textarea.value += '\\n';
                            textarea.focus();
                        ...
                        // DIAG - Diagnostics - Ver 1.9.5
                        // back_trace( 'NOTICE', 'steven_bot_bot_prompt: ' . $steven_bot_bot_prompt);
                        $rows = esc_attr(get_option('steven_bot_input_rows', '2'));
                        $steven_bot_bot_prompt = esc_attr(sanitize_text_field($steven_bot_bot_prompt));
                        // Assistant's Table Override - Ver 2.0.4
                        if ( !empty($assistant_details['placeholder_prompt']) ) {
                            $steven_bot_bot_prompt = $assistant_details['placeholder_prompt'];
                        }
                        $steven_bot_hot_bot_prompt = esc_attr(sanitize_text_field($steven_bot_hot_bot_prompt));
                        // Ver 2.5.0: Removed <center> tag and inline width style - CSS handles alignment
                        echo "<textarea id='steven-bot-message' rows='" . htmlspecialchars($rows) . "' placeholder='" . htmlspecialchars($steven_bot_bot_prompt) . "'></textarea>";
                    }
                ?>
            </div>
            <!-- Ver 2.5.0: Submit button moved inline with input for better alignment -->
            <button id="steven-bot-submit" title="Send Message">
                <img decoding="async" src="<?php echo steven_bot_appearance_icon_path('send_icon'); ?>" alt="Send">
            </button>
        </div>
        <div id="steven-bot-buttons-container">
            <?php if ($steven_bot_allow_file_uploads == 'Yes'): ?>
                <input type="file" id="steven-bot-upload-file-input" name="file[]" style="display: none;" multiple="multiple" />
                <button id="steven-bot-upload-file" title="Upload Files">
                    <img decoding="async" src="<?php echo steven_bot_appearance_icon_path('attach_icon'); ?>" alt="Upload File">
                </button>
                <script type="text/javascript">
                    document.getElementById('steven-bot-upload-file').addEventListener('click', function() {
                        document.getElementById('steven-bot-upload-file-input').click();
                    });
                </script>
            <?php endif; ?>
            <?php if ($steven_bot_allow_mp3_uploads == 'Yes'): ?>
                <input type="file" id="steven-bot-upload-mp3-input" name="file[]" style="display: none;" />
                <button id="steven-bot-upload-mp3" title="Upload an Audio/Video">
                    <img decoding="async" src="<?php echo steven_bot_appearance_icon_path('attach_icon'); ?>" alt="Upload MP3">
                </button>
                <script type="text/javascript">
                    document.getElementById('steven-bot-upload-mp3').addEventListener('click', function() {
                        document.getElementById('steven-bot-upload-mp3-input').click();
                    });
                </script>
            <?php endif; ?>
            <button id="steven-bot-erase-btn" title="Clear Conversation">
                <img decoding="async" src="<?php echo steven_bot_appearance_icon_path('erase_icon'); ?>" alt="Erase Conversation">
            </button>
            <?php if ($steven_bot_read_aloud_option == 'yes' && $voice != 'none'): ?>
                <button id="steven-bot-text-to-speech-btn" title="Read Aloud">
                    <img decoding="async" src="<?php echo steven_bot_appearance_icon_path('read_aloud_icon'); ?>" alt="Read Out Loud">
                </button>
            <?php endif; ?>
            </div>
            <?php
            $steven_bot_enable_custom_buttons = esc_attr(get_option('steven_bot_enable_custom_buttons', 'Off'));
            if ($steven_bot_enable_custom_buttons == 'Embedded' || $steven_bot_enable_custom_buttons == 'Both') {
                steven_bot_custom_buttons_display();
            }
            // Attribution - Ver 2.0.5
            steven_bot_attribution();
            ?>
        </div>
        ...
        <?php
        return ob_get_clean();
    } elseif ($steven_bot_display_style == 'floating') {
        ...
        ?>
        <div id="steven-bot">
            <div id="steven-bot-header" data-cache-buster="<?php echo time(); ?>">
                <div id="steven-bot-title" class="title"></div>
            </div>
            <div id="steven-bot-conversation"></div>
            <div id="steven-bot-input">
                <div id="steven-bot-input-area">
            ...
            <div id="steven-bot-buttons-container">
                ...
                <?php if ($steven_bot_allow_mp3_uploads == 'Yes'): ?>
                    <input type="file" id="steven-bot-upload-mp3-input" name="file[]" style="display: none;" />
                    <button id="steven-bot-upload-mp3" title="Upload MP3">
                        <img decoding="async" src="<?php echo steven_bot_appearance_icon_path('attach_icon'); ?>" alt="Upload MP3">
                    </button>
                    <script type="text/javascript">
                        document.getElementById('steven-bot-upload-mp3').addEventListener('click', function() {
            ...
            <?php
            $steven_bot_enable_custom_buttons = esc_attr(get_option('steven_bot_enable_custom_buttons', 'Off'));
            if ($steven_bot_enable_custom_buttons == 'On' || $steven_bot_enable_custom_buttons == 'Floating' || $steven_bot_enable_custom_buttons == 'Both') {
                steven_bot_custom_buttons_display();
            }
            // Attribution - Ver 2.0.5
            steven_bot_attribution();
...
function register_chatbot_shortcodes($number_of_shortcodes = null) {
    ...
    $number_of_shortcodes = get_option('steven_bot_number_of_shortcodes', null);
    ...
    if ($number_of_shortcodes === null) {
        ...
        // Cache the result
        update_option('steven_bot_number_of_shortcodes', $number_of_shortcodes);
    }
    ...
    foreach ($base_shortcodes as $shortcode) {
        if (!shortcode_exists($shortcode)) {
            add_shortcode($shortcode, 'steven_bot_shortcode');
        }
    }
...
function steven_bot_custom_buttons_display() {
    ?>
    <div id="steven-bot-custom-buttons">
        <?php
        $button_names = [];
        $button_urls = [];
        ...
        for ($i = 1; $i <= $button_count; $i++) {
            $button_names[$i] = esc_attr(get_option("steven_bot_custom_button_name_$i"));
            $button_urls[$i] = esc_attr(get_option("steven_bot_custom_button_url_$i"));
        }
        ...
        for ($i = 1; $i <= $button_count; $i++) {
            if (!empty($button_names[$i]) && !empty($button_urls[$i])) {
                ?>
                <button class="steven-bot-custom-button-class">
...
// Attribution - Ver 2.0.5
function steven_bot_attribution() {
    ...
    // back_trace( 'NOTICE', 'steven_bot_suppress_attribution: ' . $steven_bot_suppress_attribution);
    
    if ($steven_bot_suppress_attribution == 'Off') {
        if ($steven_bot_custom_attribution == 'Your custom attribution message goes here.' || empty($steven_bot_custom_attribution)) { 
            ?>
            <div class="chatbot-attribution">
                <a href="https://github.com/kelveloper" target="_blank" rel="noopener noreferrer" class="chatbot-attribution-link"><?php echo esc_html('Steven-Bot by kelveloper'); ?></a>
            </div>
            <?php
        } else {
            ?>
            <div class="chatbot-attribution">
                <p class="chatbot-attribution-text"><?php echo $steven_bot_custom_attribution; ?></p>
            </div>
            <?php
        }
    }
...
function steven_bot_shortcode_enqueue_script() {
    ...
    $style = $steven_bot_display_style ?? '';
    $assistant = $steven_bot_assistant_alias ?? '';

    // Preload avatar - Ver 2.0.3
    $avatar_icon_setting = esc_attr(get_option('steven_bot_avatar_icon_setting', ''));
    $kchat_settings['steven_bot_avatar_icon_setting'] = $avatar_icon_setting;
    $custom_avatar_icon_setting = esc_attr(get_option('steven_bot_custom_avatar_icon_setting', ''));
    $kchat_settings['steven_bot_custom_avatar_icon_setting'] = $custom_avatar_icon_setting;

    // DIAG - Diagnostics - Ver 1.9.3
    // back_trace( 'NOTICE', 'steven_bot_shortcode_enqueue_script - at the beginning of the function');
    // back_trace( 'NOTICE', 'get_the_id(): ' . get_the_id() );
    // back_trace( 'NOTICE', '$user_id: ' . $user_id);
    // back_trace( 'NOTICE', '$page_id: ' . $page_id);
    // back_trace( 'NOTICE', '$session_id: ' . $session_id);
...
add_action('wp_footer', 'steven_bot_shortcode_enqueue_script');
