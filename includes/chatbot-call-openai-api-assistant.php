<?php
/**
 * Steven-Bot - Assistants - Ver 1.6.9
 *
...
function steven_bot_get_assistant_timeout() {
    $base_timeout = intval(esc_attr(get_option('steven_bot_timeout_setting', '30')));
...
function addAMessage($thread_id, $prompt, $context, $api_key, $file_id = null, $message_uuid = null) {
    ...
    if ( !empty($file_id) ) {
        ...
        $file_type = get_steven_bot_transients_files('steven_bot_assistant_file_types', $session_id, 0);
        ...
        if ( $file_type == 'assistants' ) {
            $data = steven_bot_text_attachment($prompt, $file_id, $beta_version);
        }
        ...
        if ( $file_type == 'vision' ) {
            $data = steven_bot_image_attachment($prompt, $file_id, $beta_version);
        }
...
function runTheAssistant($thread_id, $assistant_id, $context, $api_key, $message_uuid = null) {
    ...
    $max_prompt_tokens = (int) esc_attr(get_option('steven_bot_max_prompt_tokens', 20000));
    $max_completion_tokens = (int) esc_attr(get_option('steven_bot_max_completion_tokens', 20000));
    $temperature = (float) esc_attr(get_option('steven_bot_temperature', 0.5));
    $top_p = (float) esc_attr(get_option('steven_bot_top_p', 1.0));
    ...
    if (isset($response_data['error'])) {
        ...
        if (strpos($errorMessage, 'already has an active run') !== false) {
            ...
            $locked_message = isset($steven_bot_fixed_literal_messages[19]) 
                ? $steven_bot_fixed_literal_messages[19] 
                : $default_message;
...
function getTheRunsStatus($thread_id, $runId, $api_key) {
    ...
    while ($status != "completed" && $totalRetryCount < $maxTotalRetries) {
        ...
        $response = wp_remote_get($url, [
            "headers"       => $headers,
            "timeout"       => steven_bot_get_assistant_timeout(),
        ]);
            ...
            if (isset($message['attachments']) && is_array($message['attachments'])) {
                foreach ($message['attachments'] as $attachment) {
                    if (isset($attachment['file_id'])) {
                        ...
                        set_transient('steven_bot_delete_uploaded_file_' . $file_id, $file_id, $timeFrameForDelete);
...
// Add this new function before steven_bot_custom_gpt_call_api
...
function steven_bot_custom_gpt_call_api($api_key, $message, $assistant_id, $thread_id, $session_id, $user_id, $page_id, $client_message_id = null) {
    ...
    // back_trace( 'NOTICE', 'steven_bot_custom_gpt_call_api()' );
    ...
    if (empty($thread_id)) {
        ...
        // back_trace( 'NOTICE', '$user_id ' . $user_id);
        // back_trace( 'NOTICE', '$page_id ' . $page_id);
        set_steven_bot_threads($thread_id, $assistant_id, $user_id, $page_id);
    ...
    $file_id = steven_bot_retrieve_file_id($user_id, $page_id);
    ...
    // back_trace( 'NOTICE', 'steven_bot_retrieve_file_id(): ' . print_r($file_id, true));

    // ENHANCED CONTEXT - Select some context to send with the message - Ver 2.2.4
    $use_enhanced_content_search = esc_attr(get_option('steven_bot_use_advanced_content_search', 'No'));

    // ENHANCED CONTEXT - Select some context to send with the message - Ver 2.2.4 - Updated Ver 2.2.9
    // if ($use_enhanced_content_search == 'Yes') {

    //     $search_results = steven_bot_content_search($message);
  ...
  // back_trace( 'NOTICE', 'Step 8: Get the Message');
    ...
    if (is_array($assistants_response) && 
        ...
        $email_address = esc_attr(get_option('steven_bot_conversation_transcript_email', ''));
    ...
    if (!empty($file_id)) {
        $counter = 0;
        $current_file_id = get_steven_bot_transients_files('steven_bot_assistant_file_ids', $session_id, $counter);
        
        while (!empty($current_file_id)) {
            // Set a transient that expires in 2 hours
            $timeFrameForDelete = time() + 2 * 60 * 60;
            set_transient('steven_bot_delete_uploaded_file_' . $current_file_id, $current_file_id, $timeFrameForDelete);
...
// -------------------------------------------------------------------------
function steven_bot_retrieve_file_id( $user_id, $page_id ) {

    // DIAG - Diagnostics - Ver 2.2.3
    // back_trace( 'NOTICE', 'steven_bot_retrieve_file_id(): ' . $user_id . ', ' . $page_id);
    ...
    $file_id = get_steven_bot_transients_files('steven_bot_assistant_file_ids', $session_id, $counter);
    $file_types = get_steven_bot_transients_files('steven_bot_assistant_file_types', $session_id, $counter);

    // DIAG - Diagnostics - Ver 2.0.3
    // back_trace( 'NOTICE', 'steven_bot_retrieve_file_id(): ' . print_r($file_id, true));
    // back_trace( 'NOTICE', 'steven_bot_retrieve_file_id(): ' . print_r($file_types, true));
    ...
    // DIAG - Diagnostics - Ver 2.2.3
    // back_trace( 'NOTICE', 'steven_bot_retrieve_file_ids(): ' . implode(',', $file_ids));

    // return implode(',', $file_ids);

    // Join the file ids into a comma-separated string and return it
    // DIAG - Diagnostics - Ver 2.2.3
    // back_trace( 'NOTICE', 'steven_bot_retrieve_file_ids(): ' . print_r($file_ids, true));
