<?php
/**
 * Steven-Bot - CSAT (Customer Satisfaction) Handler
 *
...
function steven_bot_submit_csat() {
    ...
    $csat_data = get_option('steven_bot_csat_data', array(
        'total' => 0,
        'helpful' => 0,
        'not_helpful' => 0,
        'responses' => array()
    ...
    // Save updated data
    $saved = update_option('steven_bot_csat_data', $csat_data);
...
add_action('wp_ajax_steven_bot_submit_csat', 'steven_bot_submit_csat');
add_action('wp_ajax_nopriv_steven_bot_submit_csat', 'steven_bot_submit_csat');

// Function to get CSAT statistics (for admin dashboard)
function steven_bot_get_csat_stats() {
    $csat_data = get_option('steven_bot_csat_data', array(
        'total' => 0,
        'helpful' => 0,
        'not_helpful' => 0,
...
function steven_bot_reset_csat_data() {
    delete_option('steven_bot_csat_data');
 ...
 * - Detractors: 0-6
 */
function steven_bot_submit_nps() {
    // Verify nonce
    if (!isset($_POST['chatbot_nonce']) || !wp_verify_nonce($_POST['chatbot_nonce'], 'chatbot_message_nonce')) {
        wp_send_json_error('Invalid nonce');
        return;
    ...
    $nps_data = get_option('steven_bot_nps_data', [
        'responses' => [],
        'total' => 0
    ]);
    ...
    update_option('steven_bot_nps_data', $nps_data);
...
add_action('wp_ajax_steven_bot_submit_nps', 'steven_bot_submit_nps');
add_action('wp_ajax_nopriv_steven_bot_submit_nps', 'steven_bot_submit_nps');
