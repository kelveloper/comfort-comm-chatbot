...
global $topwords, $words, $start_url, $domain, $max_top_words, $steven_bot_diagnostics, $plugin_dir_path, $results_dir_path, $steven_bot_no_of_items_analyzed;
$start_url = site_url();
$domain = parse_url($start_url, PHP_URL_HOST);
$max_top_words = esc_attr(get_option('steven_bot_kn_maximum_top_words', 25)); // Default to 25

// Knowledge Base Results
function steven_bot_kn_results_callback($run_scanner) {

    // DIAG - Diagnostics - Ver 1.6.3
    // back_trace( 'NOTICE', 'steven_bot_kn_results_callback');
    // back_trace( 'NOTICE', '$run_scanner: ' . $run_scanner);
    // back_trace( 'NOTICE', 'steven_bot_kn_schedule: ' . esc_attr(get_option('steven_bot_kn_schedule')));

    // NUCLEAR OPTION - OVERRIDE VALUE TO NO
    // update_option('steven_bot_kn_schedule', 'No');
    
    global $topWords;

    // Must be one of: Now, Hourly, Twice Daily, Weekly
    // $run_scanner = esc_attr(get_option('steven_bot_kn_schedule', 'No'));
    ...
    if (in_array($run_scanner, ['Now', 'Hourly', 'Daily', 'Twice Daily', 'Weekly', 'Disable', 'Cancel'])) {
        ...
        $steven_bot_no_of_items_analyzed = 0;
        ...
        if ($run_scanner === 'Cancel') {
            update_option( 'steven_bot_kn_schedule', 'No' );
            update_option( 'steven_bot_scan_interval', 'No Schedule' );
            update_option( 'steven_bot_kn_action', 'cancel' );
            update_option( 'steven_bot_kn_status', 'Cancelled' );
        } elseif ($run_scanner === 'Disable') {
            update_option( 'steven_bot_kn_schedule', 'Disable' );
            update_option( 'steven_bot_scan_interval', 'No Schedule' );
            update_option( 'steven_bot_kn_action', 'disable' );
            update_option( 'steven_bot_kn_status', 'Disabled' );
        } else {
            if (!wp_next_scheduled('knowledge_navigator_scan_hook')) {

                // RESET THE NO OF LINKS CRAWLED HERE
                update_option('steven_bot_no_of_items_analyzed', 0);
                ...
                if ($interval === 'Now') {
                    ...
                    update_option('steven_bot_scan_interval', $run_scanner);
                }
...
function steven_bot_kn_status_section_callback($args) {

    // See if the scanner is needs to run
    $results = steven_bot_kn_results_callback(esc_attr(get_option('steven_bot_kn_schedule')));

    // Force run the scanner
    // $results = steven_bot_kn_acquire();
    
    ?>
        <div class="wrap">
            <div style="background-color: white; border: 1px solid #ccc; padding: 10px; margin: 10px; display: inline-block;">

                <p><b>Scheduled to Run: </b><?php echo esc_attr(get_option('steven_bot_scan_interval', 'No Schedule')); ?></p>
                <p><b>Status of Last Run: </b><?php echo esc_attr(get_option('steven_bot_kn_status', 'Please select a Run Schedule below.')); ?></p>
                <p><b>Content Items Analyzed: </b><?php echo esc_attr(get_option('steven_bot_no_of_items_analyzed', 0)); ?></p>
            </div>
...
function steven_bot_kn_settings_section_callback($args) {
    ?>
...
function steven_bot_kn_get_published_post_types() {
...
add_action('admin_init', 'steven_bot_kn_register_settings');

function steven_bot_kn_register_settings() {
    // Register the section first
    add_settings_section(
        'steven_bot_kn_include_exclude_section',
        'Knowledge Base Include/Exclude Settings',
        'steven_bot_kn_include_exclude_section_callback',
        'steven-bot'
    );

    // Get all post types - do this only once
    $published_types = steven_bot_kn_get_published_post_types();
    ...
    foreach ($published_types as $type => $label) {
        ...
        $option_name = 'steven_bot_kn_include_' . $plural_type;
        
        // Register the setting
        register_setting(
            'steven_bot_knowledge_navigator',
            $option_name,
            'steven_bot_sanitize_checkbox'
        );
        
        // Add the settings field
        add_settings_field(
            'steven_bot_kn_include_' . $plural_type, // Use plural for field ID to match option_name
            'Include ' . ucfirst($label) . 's',    // Display plural in label
            'steven_bot_kn_include_post_type_callback',
            'steven-bot',
            'steven_bot_kn_include_exclude_section',
            [
    ...
    register_setting(
        'steven_bot_knowledge_navigator',
        'steven_bot_kn_include_comments',
        'steven_bot_sanitize_checkbox'
    );
    ...
    add_settings_field(
        ...
        'steven_bot_kn_include_comments_callback',
        'steven-bot',
        'steven_bot_kn_include_exclude_section'
    );
}

function steven_bot_kn_include_exclude_section_callback($args) {
    ?>
...
function steven_bot_kn_include_post_type_callback($args) {
...
function steven_bot_kn_enhanced_response_section_callback($args) {
    ?>
...
function steven_bot_kn_schedule_callback($args) {
    $steven_bot_kn_schedule = esc_attr(get_option('steven_bot_kn_schedule', 'No'));
    ?>
    <select id="steven_bot_kn_schedule" name="steven_bot_kn_schedule">
        <option value="No" <?php selected($steven_bot_kn_schedule, 'No'); ?>><?php echo esc_html('No'); ?></option>
        <option value="Now" <?php selected($steven_bot_kn_schedule, 'Now'); ?>><?php echo esc_html('Now'); ?></option>
        <option value="Hourly" <?php selected($steven_bot_kn_schedule, 'Hourly'); ?>><?php echo esc_html('Hourly'); ?></option>
        <option value="Twice Daily" <?php selected($steven_bot_kn_schedule, 'Twice Daily'); ?>><?php echo esc_html('Twice Daily'); ?></option>
        <option value="Daily" <?php selected($steven_bot_kn_schedule, 'Daily'); ?>><?php echo esc_html('Daily'); ?></option>
        <option value="Weekly" <?php selected($steven_bot_kn_schedule, 'Weekly'); ?>><?php echo esc_html('Weekly'); ?></option>
        <option value="Disable" <?php selected($steven_bot_kn_schedule, 'Disable'); ?>><?php echo esc_html('Disable'); ?></option>
        <option value="Cancel" <?php selected($steven_bot_kn_schedule, 'Cancel'); ?>><?php echo esc_html('Cancel'); ?></option>
    </select>
    <?php
}

function steven_bot_kn_maximum_top_words_callback($args) {
    $GLOBALS['max_top_words'] = intval(esc_attr(get_option('steven_bot_kn_maximum_top_words', 250)));
    ?>
    <select id="steven_bot_kn_maximum_top_words" name="steven_bot_kn_maximum_top_words">
        <?php
...
function steven_bot_kn_include_comments_callback($args) {
    $steven_bot_kn_include_comments = esc_attr(get_option('steven_bot_kn_include_comments', 'No'));
    ?>
    <select id="steven_bot_kn_include_comments" name="steven_bot_kn_include_comments">
        <option value="No" <?php selected($steven_bot_kn_include_comments, 'No'); ?>><?php echo esc_html('No'); ?></option>
        <option value="Yes" <?php selected($steven_bot_kn_include_comments, 'Yes'); ?>><?php echo esc_html('Yes'); ?></option>
    </select>
    <?php
}

function steven_bot_enhanced_response_limit_callback($args) {
    $steven_bot_enhanced_response_limit = intval(esc_attr(get_option('steven_bot_enhanced_response_limit', 3)));
    ?>
    <select id="steven_bot_enhanced_response_limit" name="steven_bot_enhanced_response_limit">
        <?php
        for ($i = 1; $i <= 10; $i++) {
            echo '<option value="' . $i . '"' . selected($steven_bot_enhanced_response_limit, $i, false) . '>' . $i . '</option>';
        }
        ?>
    </select>
    <?php
...
function steven_bot_kn_tuning_percentage_callback($args) {
    $steven_bot_kn_tuning_percentage = intval(esc_attr(get_option('steven_bot_kn_tuning_percentage', 25)));
    ?>
    <select id="steven_bot_kn_tuning_percentage" name="steven_bot_kn_tuning_percentage">
        <?php
        for ($i = 10; $i <= 100; $i += 5) {
            echo '<option value="' . $i . '"' . selected($steven_bot_kn_tuning_percentage, $i, false) . '>' . $i . '</option>';
        }
        ?>
    </select>
    <?php
...
function steven_bot_suppress_learnings_callback($args) {
    ...
    ?>
    <select id="chatgpt_suppress_learnings_setting" name = "steven_bot_suppress_learnings">
        <option value="None" <?php selected( $steven_bot_suppress_learnings, 'None' ); ?>><?php echo esc_html( 'None' ); ?></option>
        <option value="Random" <?php selected( $steven_bot_suppress_learnings, 'Random' ); ?>><?php echo esc_html( 'Random' ); ?></option>
        <option value="Custom" <?php selected( $steven_bot_suppress_learnings, 'Custom' ); ?>><?php echo esc_html( 'Custom' ); ?></option>
    </select>
    <?php
}

// Suppress Learnings Message - Ver 1.7.1
function steven_bot_custom_learnings_message_callback($args) {
    global $steven_bot_custom_learnings_message;
    $steven_bot_custom_learnings_message = esc_attr(get_option('steven_bot_custom_learnings_message', 'More information may be found here ...'));
    ?>
    <input type="text" style="width: 50%;" id="steven_bot_custom_learnings_message" name = "steven_bot_custom_learnings_message" value="<?php echo esc_attr( $steven_bot_custom_learnings_message ); ?>">
    <?php
}

// Optionally include post or page excerpts with Knowledge Base responses - Ver 2.2.1
function steven_bot_enhanced_response_include_excerpts_callback() {
    $value = esc_attr(get_option('steven_bot_enhanced_response_include_excerpts', 'No'));
    ?>
    <select id="steven_bot_enhanced_response_include_excerpts" name="steven_bot_enhanced_response_include_excerpts">
