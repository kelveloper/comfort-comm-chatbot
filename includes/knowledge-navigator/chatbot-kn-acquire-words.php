<?php
/**
 * Steven-Bot - Settings - Knowledge Navigator - Acquire Words
 *
...
function kn_acquire_words($Content, $action = 'add') {
    ...
    $current_total = esc_attr(get_option('steven_bot_kn_total_word_count', 0));
    // back_trace( 'NOTICE', Current total word count: ' . $current_total);
    
    // Get current document count
    $current_documents = esc_attr(get_option('steven_bot_kn_document_count', 0));
    ...
    if ($action === 'add') {
        ...
        update_option('steven_bot_kn_total_word_count', $new_total);
        // back_trace( 'NOTICE', 'Updated total word count: ' . $new_total);
        
        // Update document count
        $new_documents = $current_documents + 1;
        update_option('steven_bot_kn_document_count', $new_documents);
        ...
        foreach ($word_freq as $word => $count) {
            $existing = $wpdb->get_row($wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}steven_bot_knowledge_base_word_count WHERE word = %s",
                $word
            ));
            
            if ($existing) {
                $wpdb->update(
                    $wpdb->prefix . 'steven_bot_knowledge_base_word_count',
                    array(
