<?php
/**
 * Steven-Bot - Settings - Knowledge Navigator - Acquire Content Awareness
 *
...
// The results in the steven_bot_knowledge_base table.
//
...
function chatbot_kn_acquire_controller() {
    ...
    $action = esc_attr( get_option( 'steven_bot_kn_action', 'initialize' ) ); // Default to run to kick off the process

    // DIAG - Diagnostics - Ver 1.9.6
    // back_trace( 'NOTICE', 'steven_bot_kn_action: ' . $action  . ' ' . date('Y-m-d H:i:s') );
    ...
    switch ( $action ) {
        ...
        case 'phase 8':
            ...
            chatbot_kn_wrap_up();
            update_option( 'steven_bot_kn_action', 'completed' );
            break;            
        case 'completed':
            return;
        case 'cancel':
...
function chatbot_kn_initialization() {
    ...
    update_option( 'steven_bot_kn_item_count', 0 );

    // Define the batch size
    // FIXME - This should be set in the settings and default to 50
    update_option('steven_bot_kn_items_per_batch', 50); // Fetching 50 items at a time

    // Reset the steven_bot_knowledge_base table
    dbKNStore();

    // Reset the steven_bot_knowledge_base_word_count table
    dbKNStoreWordCount();

    // Reset the steven_bot_knowledge_base_tfidf table
    dbKNStoreTFIDF();

    // chatbot_kn_schedule_batch_acquisition();
    update_option( 'steven_bot_kn_action', 'phase 1' );

    // Reset steven_bot_kn_total_word_count to 0
    update_option('steven_bot_kn_total_word_count', 0);

    // Reset steven_bot_kn_document_count to 0
    update_option('steven_bot_kn_document_count', 0);
...
function chatbot_kn_reinitialization() {
    ...
    update_option('steven_bot_kn_action', 'phase 3');
...
function chatbot_kn_count_documents() {
    ...
    if (esc_attr(get_option('steven_bot_kn_include_comments', 'No')) === 'Yes') {
    ...
    update_option('steven_bot_kn_document_count', $document_count);
...
function chatbot_kn_run_phase_1() {
    ...
    // Get the item count
    $offset = esc_attr(get_option('steven_bot_kn_item_count', 0));
    $batch_size = esc_attr(get_option('steven_bot_kn_items_per_batch', 50));
    $steven_bot_no_of_items_analyzed = esc_attr(get_option('steven_bot_no_of_items_analyzed', 0));

    // Set the next starting point
    update_option('steven_bot_kn_item_count', $offset + $batch_size);
    ...
    if (empty($post_types)) {
        update_option('steven_bot_kn_action', 'phase 2');
        wp_schedule_single_event(time() + 30, 'chatbot_kn_acquire_controller');
        return;
    }
    ...
    foreach ($results as $result) {
        ...
        $steven_bot_no_of_items_analyzed++;
    }
...
function chatbot_kn_run_phase_3() {
    ...
    // Get the item count
    $offset = esc_attr(get_option('steven_bot_kn_item_count', 0)); // Default offset set to 0 if not specified
    // FIXME - This should be set in the settings and default to 50
    $batch_size = esc_attr(get_option('steven_bot_kn_items_per_batch', 50)); // Fetching 50 items at a time
    $steven_bot_no_of_items_analyzed = esc_attr(get_option('steven_bot_no_of_items_analyzed', 0));

    // DIAG - Diagnostics - Ver 1.9.6
    // back_trace( 'NOTICE', '$offset: ' . $offset );
    // back_trace( 'NOTICE', '$batch_size: ' . $batch_size );
    // back_trace( 'NOTICE', '$steven_bot_no_of_items_analyzed: ' . $steven_bot_no_of_items_analyzed );
    ...
    if ($steven_bot_kn_include_comments === 'Yes') {
        ...
        update_option( 'steven_bot_kn_action', 'phase 4' );
        // Schedule the next action
        wp_schedule_single_event( time() + 30, 'chatbot_kn_acquire_controller' );
...
function chatbot_kn_run_phase_4() {
    ...
    $max_top_words = esc_attr(get_option('steven_bot_kn_maximum_top_words', 100));
    
    // Get total document count and word count
    $totalDocumentCount = esc_attr(get_option('steven_bot_kn_document_count', 0));
    $totalWordCount = esc_attr(get_option('steven_bot_kn_total_word_count', 0));
    ...
    $results = $wpdb->get_results(
        "SELECT word, word_count, document_count FROM {$wpdb->prefix}steven_bot_knowledge_base_word_count 
    ...
    foreach ($results as $result) {
        ...
        // Store the TF-IDF in the steven_bot_knowledge_base_tfidf table
        $wpdb->insert(
            $wpdb->prefix . 'steven_bot_knowledge_base_tfidf',
            array(
                'word' => $word,
                'score' => $tfidf
            ),
    ...
    update_option('steven_bot_kn_action', 'phase 5');
    wp_schedule_single_event(time() + 30, 'chatbot_kn_acquire_controller');
}
...
function chatbot_kn_run_phase_5() {
    ...
    // chatbot_kn_schedule_batch_acquisition();
    update_option( 'steven_bot_kn_action', 'phase 6' );
...
function chatbot_kn_run_phase_6() {
    ...
    if (empty($post_types)) {
        update_option('steven_bot_kn_action', 'phase 7');
        wp_schedule_single_event(time() + 30, 'chatbot_kn_acquire_controller');
        return;
    }
    ...
    foreach ($results as $result) {
        ...
        if (!empty($Content)) {
            ...
            // Store each url, title, word and score in the steven_bot_knowledge_base table if the word is found in the TF-IDF table
            foreach ( $words as $word ) {
                $tfidf = $wpdb->get_var(
                    $wpdb->prepare(
                        "SELECT score FROM {$wpdb->prefix}steven_bot_knowledge_base_tfidf WHERE word = %s",
                        $word
                    )
                );
            ...
            $tuning_percentage = esc_attr(get_option('steven_bot_kn_tuning_percentage', 25)) / 100;
            ...
            // Store the top words in the steven_bot_knowledge_base table
            foreach ($top_words as $word => $score) {
                ...
                if ($score !== null) {
                    // Store each url, title, word and score in the steven_bot_knowledge_base table
                    $wpdb->insert(
                        $wpdb->prefix . 'steven_bot_knowledge_base',
                        array(
                            'url' => $url,
                            'title' => $title,
                            'word' => $word,
...
function chatbot_kn_output_the_results() {
    ...
    $results = $wpdb->get_results(
        "SELECT word, score FROM {$wpdb->prefix}steven_bot_knowledge_base_tfidf ORDER BY score DESC"
    );
    ...
    $steven_bot_kn_conversation_context = "This site includes references to and information about the following topics: ";

    foreach ($results as $result) {
        $steven_bot_kn_conversation_context .= $result->word . ", ";
    }
    
    $steven_bot_kn_conversation_context .= "and more.";

    // back_trace( 'NOTICE', 'steven_bot_kn_conversation_context: ' . $steven_bot_kn_conversation_context);
    
    // Save the results in the option for later use
    update_option('steven_bot_kn_conversation_context', $steven_bot_kn_conversation_context);
    ...
    // $results = $wpdb->get_results(
    //     "SELECT id, url, title, word, score FROM {$wpdb->prefix}steven_bot_knowledge_base ORDER BY title, url, score DESC"
    // );
    ...
    // chatbot_kn_schedule_batch_acquisition();
    update_option( 'steven_bot_kn_action', 'phase 8' );
...
function chatbot_kn_wrap_up() {
    ...
    // FIXME - Drop the steven_bot_knowledge_base_word_count table
    // DIAG - Diagnostics - Ver 1.9.6
    // back_trace( 'NOTICE', 'Dropping steven_bot_knowledge_base_word_count table' );
    dbKNClean();
    ...
    update_option('steven_bot_kn_results', $kn_results);

    // Notify outcome for up to 3 minutes
    set_transient('steven_bot_kn_results', $kn_results);
    ...
    update_option('steven_bot_kn_status', $status_message);
