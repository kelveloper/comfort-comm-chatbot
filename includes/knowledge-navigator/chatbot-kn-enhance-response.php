<?php
/**
 * Steven-Bot - Knowledge Navigator - Enhance Response - Ver 1.6.9 - Updated in Ver 2.2.9
 * 
 * Updates
 * 
...
function steven_bot_enhance_with_tfidf($message) {

    // DIAG - Diagnostics - Ver 2.2.9
    // back_trace( 'NOTICE', 'steven_bot_enhance_with_tfidf');
    ...
    $enhanced_response = "";
    $results = array();
    $limit = esc_attr(get_option('steven_bot_enhanced_response_limit', 3));
    ...
    $include_title = esc_attr(get_option('steven_bot_enhanced_response_include_title', 'yes'));

    // Decide if the links to site content and exceprts should be included in the response
    $include_post_or_page_excerpt = esc_attr(get_option('steven_bot_enhanced_response_include_excerpts', 'No'));
    ...
    if (!empty($links)) {
        ...
        if ('Random' == $steven_bot_suppress_learnings) {
            $enhanced_response .= "\n\n" . $localized_learningMessages[array_rand($localized_learningMessages)] . " ";
        } elseif ('Custom' == $steven_bot_suppress_learnings) {
...
function steven_bot_enhance_with_tfidf_deprecated($message) {
    ...
    $steven_bot_kn_status = esc_attr(get_option('steven_bot_kn_status', ''));
    if (false === strpos($steven_bot_kn_status, 'Completed')) {
        return;
    }

    $table_name = $wpdb->prefix . 'steven_bot_knowledge_base';
