...
function steven_bot_kn_analysis_section_callback($args) {
    ?>
    <p>Use the 'Download Data' button to retrieve the Knowledge Base results.</p>
    <p style="background-color: #e0f7fa; padding: 10px;"><b>For an explanation on how to use the Knowledge Base Analysis and additional documentation please click <a href="?page=steven-bot&tab=support&dir=knowledge-navigator&file=knowledge-navigator-analysis.md">here</a>.</b></p>
    <?php
    if (is_admin()) {
        $header = " ";
        $header .= '<a class="button button-primary" href="' . esc_url(admin_url('admin-post.php?action=steven_bot_kn_analysis_download_csv')) . '">Download Data</a>';
        echo $header;
    }
}
...
// Knowledge Base Analysis section callback - Ver 1.6.2
function steven_bot_kn_analysis_output_callback($args) {
    // Get the saved steven_bot_kn_analysis_choice value or default to "CSV"
    $output_choice = esc_attr(get_option('steven_bot_kn_analysis_output', 'CSV'));
    // DIAG - Log the output choice
    // back_trace( 'NOTICE', '$output_choice' . $output_choice);
    ?>
    <select id="steven_bot_kn_analysis_output" name="steven_bot_kn_analysis_output">
...
function steven_bot_kn_analysis_download_csv() {
...
add_action('admin_post_steven_bot_kn_analysis_download_csv', 'steven_bot_kn_analysis_download_csv');
