...
function steven_bot_add_dashboard_widget() {

    // Only show to users who can manage options (administrators)
    if (current_user_can('manage_options')) {
        wp_add_dashboard_widget(
            'steven_bot_dashboard_widget',
            'Steven-Bot Statistics',
            'steven_bot_dashboard_widget_content'
        );
    }

}
add_action('wp_dashboard_setup', 'steven_bot_add_dashboard_widget');
...
function steven_bot_dashboard_widget_content() {

    // Get the current period setting
    $period = get_option('steven_bot_dashboard_period', '24h');
    $view_type = get_option('steven_bot_dashboard_view', 'sessions');

    // Handle form submission
    if (isset($_POST['steven_bot_dashboard_period'])) {
        $period = sanitize_text_field($_POST['steven_bot_dashboard_period']);
        update_option('steven_bot_dashboard_period', $period);
    }

    if (isset($_POST['steven_bot_dashboard_view'])) {
        $view_type = sanitize_text_field($_POST['steven_bot_dashboard_view']);
        update_option('steven_bot_dashboard_view', $view_type);
    }
    ...
    <div class="chatbot-dashboard-stats">
        ...
        <form method="post" class="chatbot-period-selector">
            <label for="steven_bot_dashboard_period">Time Period:</label>
            <div class="chatbot-period-select-wrapper">
                <select name="steven_bot_dashboard_period" id="steven_bot_dashboard_period" onchange="this.form.submit()" class="chatbot-period-select">
        ...
        <form method="post" class="chatbot-view-selector">
            <label for="steven_bot_dashboard_view">View Type:</label>
            <div class="chatbot-view-select-wrapper">
                <select name="steven_bot_dashboard_view" id="steven_bot_dashboard_view" onchange="this.form.submit()" class="chatbot-view-select">
