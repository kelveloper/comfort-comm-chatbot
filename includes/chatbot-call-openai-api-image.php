<?php
/**
 * Steven-Bot - ChatGPT IMAGE API - Ver 1.9.4
 *
...
function steven_bot_call_image_api($api_key, $message, $user_id = null, $page_id = null, $session_id = null, $assistant_id = null, $client_message_id = null) {
    ...
    if ( !empty($kchat_settings['model']) ) {
        ...
        $model = esc_attr(get_option('steven_bot_image_model_option', 'dall-e-2'));
        // DIAG - Diagnostics - Ver 1.9.4
    ...
    $quantity = intval(esc_attr(get_option('steven_bot_image_output_quantity', '1')));
    ...
    $size = esc_attr(get_option('steven_bot_image_output_size', '1024x1024'));
    ...
    $quality = esc_attr(get_option('steven_bot_image_quality_output', 'standard'));
    $style = esc_attr(get_option('steven_bot_image_style_output', 'vivid'));
    ...
    if (is_wp_error($response)) {
        prod_trace( 'ERROR', 'steven_bot_call_image_api() - Error: ' . $response->get_error_message());
        // Clear locks on error
        // Lock clearing removed - main send function handles locking
    ...
    if ($http_code !== 200 || isset($response_body['error'])) {
        ...
        prod_trace( 'ERROR', 'steven_bot_call_image_api() - Error: API responded with HTTP code ' . $http_code . ': ' . $error_message);
        // Clear locks on error
        // Lock clearing removed - main send function handles locking
