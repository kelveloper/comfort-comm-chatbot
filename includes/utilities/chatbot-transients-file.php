...
function set_steven_bot_transients_files( $transient_type, $transient_value, $session_id, $file_no ) {
    ...
    if ($transient_type == 'steven_bot_assistant_file_ids') {
        $transient_key = 'steven_bot_assistant_file_ids_' . $session_id . '_' . $file_no;
    } elseif ($transient_type == 'steven_bot_assistant_file_types') {
        $transient_key = 'steven_bot_assistant_file_types_' . $session_id . '_' . $file_no;
    } else {
        $transient_key = 'steven_bot_file_id_' . $session_id . '_' . $file_no;
    }
...
function get_steven_bot_transients_files( $transient_type, $session_id, $file_no ): string {
...
function delete_steven_bot_transients_files( $transient_type, $session_id, $file_no ) {
    ...
    // Construct the transient key based on the transient type
    if ($transient_type == 'steven_bot_assistant_file_ids') {
        $file_transient_key = 'steven_bot_assistant_file_ids_' . $session_id . '_' . $file_no;
    } elseif ($transient_type == 'steven_bot_assistant_file_types') {
        $file_transient_key = 'steven_bot_assistant_file_types_' . $session_id . '_' . $file_no;
    } else {
...
function steven_bot_cleanup_old_file_transients($session_id) {
    
    // Delete old format transients for this session
    for ($i = 0; $i < 10; $i++) {
        $old_key = 'steven_bot_file_id_' . $session_id . '_' . $i;
    ...
    ", '_transient_steven_bot_file_id_%', '%' . $session_id . '%'));
}
