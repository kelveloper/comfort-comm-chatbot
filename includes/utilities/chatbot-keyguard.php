<?php
/**
 * Steven-Bot - Keyguard Support - Ver 2.2.6
 *
...
// steven_bot_upgrade_encrypted_api_keys('steven_bot_fetch_stored_api_keys', 'steven_bot_update_stored_api_key', $old_keyguard);

// XORs the given key with a salt key and returns the base64-encoded result - Ver 2.2.6
function steven_bot_obfuscate_keyguard($key) {
...
function steven_bot_deobfuscate_keyguard($obfuscated_key) {
...
function steven_bot_generate_and_store_keyguard() {
    ...
    if (empty($obfuscated_key)) {
        ...
        // Obfuscate the key.
        $obfuscated_key = steven_bot_obfuscate_keyguard($key);
    ...
    return steven_bot_deobfuscate_keyguard($obfuscated_key);
...
//
function steven_bot_encrypt_api_key_with_key($plaintext, $keyguard) {
...
function steven_bot_decrypt_api_key_with_key($data, $keyguard) {
...
function steven_bot_encrypt_api_key($plaintext) {

    $keyguard = steven_bot_generate_and_store_keyguard();
    return steven_bot_encrypt_api_key_with_key($plaintext, $keyguard);

}

// Convenience function to decrypt an API key using the stored secret key - Ver 2.2.6
function steven_bot_decrypt_api_key($data, $option_name = null) {

    // Fix the JSON before decrypting.
    $data = html_entity_decode($data);
    // error_log('[Chatbot] [chatbot-keyguard.php] steven_bot_decrypt_api_key() - $data: ' . print_r($data, true));
    ...
    if (!is_array($decoded) || !isset($decoded['iv']) || !isset($decoded['encrypted'])) {
        ...
        switch ($chatbot_ai_platform_choice) {
            case 'OpenAI':
                $option_name = 'steven_bot_api_key';
                break;
            case 'NVIDIA':
        ...
        if (!empty($option_name)) {
            $encrypted_value = steven_bot_encrypt_api_key($data);
    ...
    $keyguard = steven_bot_generate_and_store_keyguard();
    // error_log('[Chatbot] [chatbot-keyguard.php] steven_bot_decrypt_api_key() - $keyguard: ' . $keyguard);
    $decrypted_key = steven_bot_decrypt_api_key_with_key($data, $keyguard);
    // error_log('[Chatbot] [chatbot-keyguard.php] steven_bot_decrypt_api_key() - $decrypted_key: ' . $decrypted_key);
...
// with the new secret key (retrieved via steven_bot_generate_and_store_keyguard()). The function takes two callbacks:
// one to fetch all stored keys and one to update each key in storage.
//
function steven_bot_upgrade_encrypted_api_keys(callable $fetch_keys_callback, callable $update_key_callback, $old_keyguard) {

    // Get the new secret key (this will generate one if it doesn't exist).
    $new_keyguard = steven_bot_generate_and_store_keyguard();
    ...
    foreach ($stored_keys as $key_id => $encrypted_api_key) {
        ...
        $decrypted = steven_bot_decrypt_api_key_with_key($encrypted_api_key, $old_keyguard);
        if ($decrypted === false) {
            // Log or handle the decryption error, then skip this key.
            continue;
        }
        // Re-encrypt using the new secret key.
        $new_encrypted = steven_bot_encrypt_api_key_with_key($decrypted, $new_keyguard);
...
//     ];
//
function steven_bot_fetch_stored_api_keys($chatbot_ai_platform_choice = null) {
    ...
    $stored_keys = array();
    $stored_keys = array(
        'steven_bot_api_key' => esc_attr( get_option('steven_bot_api_key') ),
    ...
    // DIAG - Diagnostics
    // error_log('chatbot-keyguard.php: steven_bot_fetch_stored_api_keys() - $stored_keys: ' . print_r($stored_keys, true));
...
function steven_bot_update_stored_api_key($key_id, $new_encrypted_value) {
    ...
    if ($key_id == 'steven_bot_api_key') {
        update_option('steven_bot_api_key', $new_encrypted_value);
...
//
function steven_bot_upgrade_unencrypted_api_keys(callable $fetch_keys_callback, callable $update_key_callback) {
    ...
    foreach ($stored_keys as $key_id => $stored_value) {
        ...
        $new_encrypted_value = steven_bot_encrypt_api_key_with_key($stored_value, $keyguard);
...
add_action('admin_init', 'steven_bot_run_chatbot_api_key_upgrade');

// Force the upgrade process to run - Ver 2.2.6
function steven_bot_run_chatbot_api_key_upgrade() {

    // Check if the upgrade has already run.
    if ( get_option('kognetiks_keyguard_upgraded') !== 'yes' ) {
        // For unencrypted keys, run the upgrade.
        steven_bot_upgrade_unencrypted_api_keys('steven_bot_fetch_stored_api_keys', 'steven_bot_update_stored_api_key');
        
        // Optionally, if you need to upgrade keys encrypted with an old secret key,
        // call steven_bot_upgrade_encrypted_api_keys here as well:
        // steven_bot_upgrade_encrypted_api_keys('steven_bot_fetch_stored_api_keys', 'steven_bot_update_stored_api_key', $old_keyguard);
...
function steven_bot_sanitize_api_key($input) {
    ...
    if (!empty($input)) {
        $encrypted_key = steven_bot_encrypt_api_key($input);
