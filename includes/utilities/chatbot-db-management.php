<?php
/**
 * Steven-Bot - Database Management for Reporting - Ver 1.6.3
 *
...
function create_steven_bot_interactions_table() {
    
    global $wpdb;

    $table_name = $wpdb->prefix . 'steven_bot_interactions';
    ...
    // DIAG - Diagnostics
    // back_trace( 'SUCCESS', 'Successfully created steven_bot_interactions table');
    return;

}
// Hook to run the function when the plugin is activated
// register_activation_hook(__FILE__, 'create_steven_bot_interactions_table');
...
function create_conversation_logging_table() {
    ...
    if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_name)) === $table_name) {
        ...
        if ($wpdb->get_var($wpdb->prepare("SHOW COLUMNS FROM $table_name LIKE %s", 'assistant_name')) === 'assistant_name') {
            ...
            if ($result === false) {
                // If there was an error, log it
                // back_trace( 'ERROR', 'Error altering steven_bot_conversation_log table: ' . $wpdb->last_error);
            } else {
                // If the operation was successful, log the success
                // back_trace( 'SUCCESS', 'Successfully altered steven_bot_conversation_log table');
            }
        }
        ...
        foreach ($rows as $row) {
            ...
            if ($update_result === false) {
                // If there was an error, log it
                // back_trace( 'ERROR', 'Error updating missing steven_bot_conversation_log table: ' . $wpdb->last_error);
            } else {
                // If the operation was successful, log the success
                // back_trace( 'SUCCESS', 'Successfully updated missing values in steven_bot_conversation_log table');
            }
        }
        
        // DIAG - Diagnostics - Ver 1.9.9
        // back_trace( 'SUCCESS', 'Successfully updated steven_bot_conversation_log table');
    ...
    // back_trace( 'SUCCESS', 'Successfully created/updated steven_bot_conversation_log table');
...
function append_message_to_conversation_log($session_id, $user_id, $page_id, $user_type, $thread_id, $assistant_id, $assistant_name, $message) {
    ...
    $assistant_name = get_steven_bot_transients('assistant_name', $user_id, $page_id, $session_id);
...
// Updated Ver 2.4.8: Uses Supabase only
function steven_bot_conversation_log_cleanup() {

    // Get the number of days to keep the conversation log
    $days_to_keep = esc_attr(get_option('steven_bot_conversation_log_days_to_keep'));
...
register_activation_hook(plugin_dir_path(dirname(__FILE__)) . 'steven-bot.php', 'steven_bot_activate_db');
register_deactivation_hook(plugin_dir_path(dirname(__FILE__)) . 'steven-bot.php', 'steven_bot_deactivate_db');

// Function to handle database setup on activation
function steven_bot_activate_db() {
...
function steven_bot_deactivate_db() {
    // Clear the scheduled cleanup event
    wp_clear_scheduled_hook('steven_bot_conversation_log_cleanup_event');
