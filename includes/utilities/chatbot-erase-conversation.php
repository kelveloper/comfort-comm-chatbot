<?php
/**
 * Steven-Bot - Clear Conversation - Ver 1.8.6
 *
...
function steven_bot_erase_conversation_handler() {
    ...
    $steven_bot_force_page_reload = isset($_POST['steven_bot_force_page_reload']) ? sanitize_text_field($_POST['steven_bot_force_page_reload']) : 'Yes';
    ...
    $assistant_id = get_steven_bot_transients( $transient_type , $user_id, $page_id, $session_id);
    ...
    // Wipe the Context
    update_option( 'steven_bot_conversation_context' ,'' , true);

    delete_steven_bot_threads($user_id, $page_id);
    ...
    // back_trace( 'NOTICE', '$steven_bot_force_page_reload: ' . $steven_bot_force_page_reload);

    if ($steven_bot_force_page_reload == 'Yes') {
        ...
        $success_message = isset($steven_bot_fixed_literal_messages[16])
            ? $steven_bot_fixed_literal_messages[16]
            : $default_message;
        // Send error response
        ...
        $success_message = isset($steven_bot_fixed_literal_messages[17])
            ? $steven_bot_fixed_literal_messages[17]
            : $default_message;
        // Send error response
        wp_send_json_success($success_message);
    }
    ...
    $error_message = isset($steven_bot_fixed_literal_messages[18])
        ? $steven_bot_fixed_literal_messages[18]
        : $default_message;
    // Send error response
...
function delete_any_file_transients($session_id) {
    ...
    $sql = "DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_steven_bot_file_id_$session_id%'";
    $wpdb->query($sql);
...
function verify_session_ownership($session_id) {
    ...
    // which creates strings like "kognetiks_1234567890.123456" or "steven_1234567890.123456"
    if (!preg_match('/^(kognetiks|steven)_[a-zA-Z0-9._]+$/', $session_id)) {
        return false;
    }
    
    // Additional security: Check if session_id is reasonable length (not too short or too long)
    // Session IDs are typically around 25-30 characters (e.g., "kognetiks_1234567890.123456")
    if (strlen($session_id) < 15 || strlen($session_id) > 64) {
        return false;
    }
