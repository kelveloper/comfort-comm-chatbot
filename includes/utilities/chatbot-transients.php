...
function steven_bot_set_consolidated_transient_data ( $page_id , $user_id , $session_id , $consolidated_transient_data ) {
    ...
    //     'user_id' => $user_id,
    //     'page_id' => $page_id,
    //     'assistant_alias' => isset( $steven_bot_assistant_alias ) ? $steven_bot_assistant_alias : '',
    //     'file_id' => isset( $file_id ) ? $file_id : '',
    //     'display_style' => isset( $steven_bot_display_style ) ? $steven_bot_display_style : '',
    ...
    // steven_bot_set_consolidated_transient_data( $page_id , $user_id , $session_id, $consolidated_transient_data );

    // DIAG - Diagnostics - Ver 2.2.7
    // back_trace( 'NOTICE', 'steven_bot_set_consolidated_transient_data - START');
    ...
    // Store the transient
    set_transient('steven_bot_transient_' . $page_id . '_' . $user_id , $consolidated_transient_data, 60*60*24); // Store for 24 hours   

    // DIAG - Diagnostics - Ver 2.2.7
    // back_trace( 'NOTICE', 'steven_bot_set_consolidated_transient_data - END');

}

// Consolidated Transient Get Functions - Ver 2.2.7
function steven_bot_get_consolidated_transient_data ( $page_id , $user_id , $session_id  ){
    ...
    // $consolidated_transient_data = steven_bot_get_consolidated_transient_data( $page_id , $user_id );
    ...
    // DIAG - Diagnostics - Ver 2.2.7
    // back_trace( 'NOTICE', 'steven_bot_get_consolidated_transient_data - START');
    // back_trace( 'NOTICE', '$user_id: ' . $user_id);
    // back_trace( 'NOTICE', '$page_id: ' . $page_id);
    // back_trace( 'NOTICE', '$session_id: ' . $session_id);
    ...
    $consolidated_transient_data = get_transient('steven_bot_transient_' . $page_id . '_' . $user_id);
...
function set_steven_bot_transients( $transient_type , $transient_value , $user_id = null, $page_id = null, $session_id = null, $thread_id = null, $sequence_id = null, $step_id = null) {
    ...
    if ( $transient_type == 'display_style' ) {
        $transient_key = 'steven_bot_style_' . $page_id . '_' . $session_id;
    } elseif (  $transient_type== 'assistant_alias' ) {
        $transient_key = 'steven_bot_assistant_' . $page_id . '_' . $session_id;
    } elseif ( $transient_type == 'file_id' ) {
        $transient_key = 'steven_bot_file_id_' . $session_id;
    } elseif ( $transient_type == 'steven_bot_assistant_file_id') {
        $transient_key = 'steven_bot_assistant_file_id_' . $session_id;
    } elseif ( $transient_type == 'model' ) {
        $transient_key = 'steven_bot_model_' . $page_id . '_' . $session_id;
    } elseif ( $transient_type == 'voice') {
        $transient_key = 'steven_bot_voice_' . $page_id . '_' . $session_id;
    } elseif ( $transient_type == 'kflow_sequence' ) {
        $transient_key = 'steven_bot_kflow_sequence_' . $session_id;
    } elseif ( $transient_type == 'kflow_step' ) {
        $transient_key = 'steven_bot_kflow_step_' . $session_id;
    } elseif ( $transient_type == 'assistant_name' ) {
        $transient_key = 'steven_bot_assistant_name_' . $page_id . '_' . $session_id;
    } elseif ( $transient_type == 'assistant_id' ) {
        $transient_key = 'steven_bot_assistant_id_' . $page_id . '_' . $session_id;
    } elseif ( $transient_type == 'thread_id' ) {
        $transient_key = 'steven_bot_thread_id_' . $page_id . '_' . $session_id;
    } elseif ( $transient_type == 'additional_instructions' ) {
        $transient_key = 'steven_bot_additional_instructions_' . $page_id . '_' . $session_id;
    } else {
        return;
    }
...
// Updated Ver 2.3.6: Always use session_id in transient keys for consistency between logged-in and anonymous users
function get_steven_bot_transients( $transient_type, $user_id = null, $page_id = null, $session_id = null ): string {
    ...
    if ($transient_type == 'file_id' || $transient_type == 'steven_bot_assistant_file_id') {
        // back_trace( 'NOTICE', '$session_id ' . $session_id);
    } else {
        // back_trace( 'NOTICE', '$session_id ' . $session_id);
        // back_trace( 'NOTICE', '$page_id ' . $page_id);
...
function delete_steven_bot_transients( $transient_type, $user_id = null, $page_id = null, $session_id = null ) {
    ...
    if ( $transient_type == 'display_style' ) {
        $style_transient_key = 'steven_bot_style_' . $user_id . '_' . $page_id;
        delete_transient($style_transient_key);
    } elseif ( $transient_type == 'assistant_alias' ) {
        $assistant_transient_key = 'steven_bot_assistant_id_' . $user_id . '_' . $page_id;
        delete_transient($assistant_transient_key);
    } elseif ( $transient_type == 'steven_bot_file_id' ) {
        $file_transient_key = 'steven_bot_file_id_' . $session_id . '_' . $file_no;
        delete_transient($file_transient_key);
    } elseif ( $transient_type == 'steven_bot_assistant_file_id') {
        $asst_file_transient_key = 'steven_bot_assistant_file_id_' . $session_id;
        delete_transient($asst_file_transient_key);
    } elseif ( $transient_type == 'model' ) {
        $model_transient_key = 'steven_bot_model_' . $user_id . '_' . $page_id;
        delete_transient($model_transient_key);
    } elseif ( $transient_type == 'voice' ) {
        $voice_transient_key = 'steven_bot_voice_' . $user_id . '_' . $page_id;
        delete_transient($voice_transient_key);
    } elseif ( $transient_type == 'kflow_sequence' ) {
        $kflow_sequence_transient_key = 'steven_bot_kflow_sequence_' . $session_id;
        delete_transient($kflow_sequence_transient_key);
    } elseif ( $transient_type == 'kflow_step' ) {
        $kflow_step_transient_key = 'steven_bot_kflow_step_' . $session_id;
        delete_transient($kflow_step_transient_key);
    } elseif ( $transient_type == 'assistant_name' ) {
        $assistant_name_transient_key = 'steven_bot_assistant_name_' . $user_id . '_' . $page_id;
        delete_transient($assistant_name_transient_key);
    } elseif ( $transient_type == 'assistant_id' ) {
        $assistant_id_transient_key = 'steven_bot_assistant_id_' . $user_id . '_' . $page_id;
        delete_transient($assistant_id_transient_key);
    } elseif ( $transient_type == 'thread_id' ) {
        $thread_id_transient_key = 'steven_bot_thread_id_' . $user_id . '_' . $page_id;
        delete_transient($thread_id_transient_key);
    } elseif ( $transient_type == 'additional_instructions' ) {
        $additional_instructions_transient_key = 'steven_bot_additional_instructions_' . $user_id . '_' . $page_id;
