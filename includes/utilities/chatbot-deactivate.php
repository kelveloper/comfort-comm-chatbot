<?php
/**
 * Steven-Bot - Deactivate and/or Delete the Plugin
 *
...
function steven_bot_deactivate() {

    if (empty(esc_attr(get_option('steven_bot_delete_data')))) {      
        steven_bot_admin_notices();
    }

}

// Delete Plugin Data Notice - Ver 1.9.9
add_action('admin_notices', 'steven_bot_admin_notices');
function steven_bot_admin_notices() {

    if (empty(esc_attr(get_option('steven_bot_delete_data')))) {     

        echo '<div class="notice notice-warning is-dismissible">
            <p><strong>Steven-Bot:</strong> Remember to set your data deletion preferences in the plugin settings on the Messages tab if you plan to uninstall the plugin.</p>
        </div>';
        update_option('steven_bot_delete_data', 'no');
...
function steven_bot_uninstall(){
    ...
    if (esc_attr(get_option('steven_bot_delete_data')) != 'yes') {
        return;
    }
    ...
    if (esc_attr(get_option('steven_bot_delete_data')) == 'yes') {
        ...
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_assistants");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_azure_assistants");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_conversation_log");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_interactions");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_knowledge_base");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_knowledge_base_tfidf");
        $wpdb->query("DROP TABLE IF EXISTS {$wpdb->prefix}steven_bot_knowledge_base_word_count");
