<?php
/**
 * Steven-Bot - File Uploads - Ver 1.7.6 - Updated for Ver 2.0.1
 *
...
function steven_bot_upload_files() {
    ...
    $uploads_dir = $steven_bot_plugin_dir_path . 'uploads/';
    ...
    if (!file_exists($uploads_dir) && !wp_mkdir_p($uploads_dir)) {
        ...
        $error_message = !empty($steven_bot_fixed_literal_messages[2]) 
            ? $steven_bot_fixed_literal_messages[2] 
            : $default_message;
        $responses[] = array(
            'status' => 'error',
    ...
    if (empty($api_key)) {
        ...
        $error_message = !empty($steven_bot_fixed_literal_messages[3]) 
            ? $steven_bot_fixed_literal_messages[3] 
            : $default_message;
        $responses[] = array(
            'status' => 'error',
            'message' => $error_message
    ...
    if (isset($_FILES['file']['name']) && is_array($_FILES['file']['name'])) {
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if ($_FILES['file']['error'][$i] > 0) {
                $error_message = !empty($steven_bot_fixed_literal_messages[4]) 
                    ? $steven_bot_fixed_literal_messages[4] 
            ...
            // Move file to uploads directory using WordPress-approved method
            if (!steven_bot_move_uploaded_file($_FILES['file']['tmp_name'][$i], $file_path)) {
                $error_message = !empty($steven_bot_fixed_literal_messages[4]) 
                    ? $steven_bot_fixed_literal_messages[4] 
                    : "Oops! Something went wrong during the upload of {$_FILES['file']['name'][$i]}. Please try again later.";
            ...
            // Store API response
            set_steven_bot_transients_files('steven_bot_assistant_file_ids', $responseData['id'], $session_id, $i);
            set_steven_bot_transients_files('steven_bot_assistant_file_types', $purpose, $session_id, $i);
            
            // Clean up old transient keys to prevent conflicts
            steven_bot_cleanup_old_file_transients($session_id);
        ...
        $error_message = isset($steven_bot_fixed_literal_messages[5]) 
            ? $steven_bot_fixed_literal_messages[5] 
            : $default_message;
        return array(
            'status' => 'error',
            'message' => $error_message
...
function steven_bot_upload_mp3() {
    ...
    if (isset($_FILES['file']['name']) && is_array($_FILES['file']['name'])) {
        for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
            ...
            if ($_FILES['file']['error'][$i] > 0) {
                ...
                $error_message = isset($steven_bot_fixed_literal_messages[4]) 
                    ? $steven_bot_fixed_literal_messages[4] 
                    : $default_message;
                $responses[] = array(
                    'status' => 'error',
        ...
        set_steven_bot_transients_files('steven_bot_assistant_file_ids', $newFileName, $session_id, $i);
        set_steven_bot_transients_files('steven_bot_assistant_file_types', 'mp3', $session_id, $i);
        $responses[] = array(
            'status' => 'success',
...
function steven_bot_cleanup_uploads_directory() {
...
add_action('steven_bot_cleanup_upload_files', 'steven_bot_cleanup_uploads_directory');
...
/**
 * WordPress-approved file upload function
 * Replaces move_uploaded_file() with copy() for WordPress.org compliance
 *
 * @param string $tmp_name The temporary file path
 * @param string $destination The destination file path
 * @return bool True on success, false on failure
 */
function steven_bot_move_uploaded_file($tmp_name, $destination) {
    // Verify it's a valid uploaded file
    if (!is_uploaded_file($tmp_name)) {
        return false;
    }

    // Use copy instead of move_uploaded_file (WordPress-approved)
    $result = copy($tmp_name, $destination);

    // Clean up the temp file
    if ($result && file_exists($tmp_name)) {
        @unlink($tmp_name);
    }
