<?php
/**
 * Steven-Bot - Chatbot Conversation History
 *
...
// Usage: [chat_history] or [chatbot_conversation] or [steven_bot_history]
...
function interactive_chat_history() {
    ...
    $output = '<div class="steven-bot-chatbot-history">';
    foreach ($grouped_conversations as $thread_id => $messages) {
        ...
        $output .= sprintf('<div class="steven-bot-chatbot-history" id="thread-%s">', esc_attr($thread_id));
        ...
        foreach ($messages as $message) {
            ...
            if (empty($assistant_name)) {
                $assistant_name = esc_attr(get_option('steven_bot_bot_name'));
            }
...
add_shortcode('steven_bot_history', 'interactive_chat_history');
...
function steven_bot_get_converation_history($session_id) {
    ...
    $context_history = get_transient('steven_bot_context_history');
