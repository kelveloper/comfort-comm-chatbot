<?php
/**
 * Steven-Bot - Google Gemini API - Ver 2.3.7
 *
...
- Service cancellations';
    ...
    $steven_bot_kn_conversation_context = esc_attr(get_option('steven_bot_kn_conversation_context', 'Yes'));
    ...
    if ($use_enhanced_content_search == 'Yes') {
        ...
        if ($is_new_conversation_greeting) {
            $context = $context . ' ' . $steven_bot_kn_conversation_context;
        } else {
    ...
    if ($steven_bot_conversation_continuation == 'On' && !$is_new_conversation_greeting) {
    ...
    if (isset($response_body->error)) {
        ...
        // Check for rate limit / quota errors and provide helpful message
        if ($error_code == 429 || strpos(strtolower($error_message), 'quota') !== false || strpos(strtolower($error_message), 'rate') !== false) {
            return "I'm sorry, I couldn't find an answer to that question in our knowledge base. Please try asking about our services, pricing, or contact information.\n\nFor immediate assistance, call us at (347) 519-9999 or email stevenj@comfort-business.com.";
        }
    ...
    if (isset($response_body->candidates[0]->content->parts[0]->text) && !empty($response_body->candidates[0]->content->parts[0]->text)) {
        ...
        addEntry('steven_bot_context_history', $response_text);
