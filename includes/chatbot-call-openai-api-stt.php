<?php
/**
 * Steven-Bot - ChatGPT STT API - Ver 2.2.3
 *
...
function steven_bot_call_stt_api($api_key, $message, $stt_option = null, $user_id = null, $page_id = null, $session_id = null, $assistant_id = null, $client_message_id = null) {

    // DIAG - Diagnostics
    // back_trace( 'NOTICE', 'steven_bot_call_stt_api()' );
    ...
    $counter = 1;
    $audio_file_name = get_steven_bot_transients_files('steven_bot_assistant_file_ids', $session_id, $counter);
    $audio_file_name = $steven_bot_plugin_dir_path . 'uploads/' . $audio_file_name;
    ...
    $body .= esc_attr(get_option('steven_bot_whisper_model_option', 'whisper-1')) . "\r\n";
    $body .= "--{$boundary}\r\n";
    ...
    $result = steven_bot_post_process_transcription($api_key, $message, $transcription);
    // Clear locks on success
...
function steven_bot_post_process_transcription($api_key, $message, $transcription) {

    // DIAG - Diagnostics
    // back_trace( 'NOTICE', 'steven_bot_post_process_transcription()' );

    // Get API URL for text processing
    $api_url = get_chat_completions_api_url();
    $model = esc_attr(get_option('steven_bot_model_choice', 'gpt-3.5-turbo'));
    $max_tokens = intval(get_option('steven_bot_max_tokens_setting', 500));
