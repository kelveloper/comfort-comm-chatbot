<?php
/**
 * Steven-Bot - ChatGPT TTS API - Ver 1.9.4
 *
...
function steven_bot_call_tts_api($api_key, $message, $voice = null, $user_id = null, $page_id = null, $session_id = null, $assistant_id = null, $client_message_id = null) {
    ...
    $audio_dir_path = $steven_bot_plugin_dir_path . 'audio/';
    ...
    $audio_format = esc_attr(get_option('steven_bot_audio_output_format', 'mp3'));
    ...
    $audio_file_url = plugins_url('audio/' . $audio_file_name, $steven_bot_plugin_dir_path . 'steven-bot');
    ...
    if ( !empty($kchat_settings['model']) ) {
        ...
        $model = esc_attr(get_option('steven_bot_model_choice', 'tts-1-hd'));
        // DIAG - Diagnostics - Ver 1.9.4
    ...
    if ( empty($voice) ) {
        $voice = get_steven_bot_transients( 'voice', $user_id, $page_id, $session_id);
    }
    ...
    if ( empty($model) ) {
        $model = 'tts-1-hd';
        update_option( 'steven_bot_model_choice', 'tts-1-hd');
    }
    if ( empty($voice) ) {
        $voice = 'alloy';
        update_option( 'steven_bot_voice_option', 'alloy');
    }
    ...
    if (!empty($audio_output)) {
        ...
        steven_bot_delete_audio_file_id( $audio_file_url );
...
function steven_bot_read_aloud($message) {
    ...
    // Hold the model
    // $t_model = get_steven_bot_transients( 'model', $user_id, $page_id, $session_id);
    // if ( empty($t_model) ) {
    //     $t_model = esc_attr(get_option( 'steven_bot_voice_model_option', 'tts-1-hd'));
    // }
    // $model = $t_model;
    // set_steven_bot_transients( 'model', $model, $user_id, $page_id, $session_id);
    $kchat_settings['model'] = esc_attr(get_option( 'steven_bot_voice_model_option', 'tts-1-hd'));

    // FIXME - READ ALOUD PASSED FROM ASSISTANT MANAGEMENT

    // Hold the voice
    $t_voice = get_steven_bot_transients( 'voice', $user_id, $page_id, $session_id);
    if ( empty($t_voice) ) {
        $t_voice = esc_attr(get_option( 'steven_bot_voice_option', 'alloy') );
    }
    $voice = $t_voice;
    set_steven_bot_transients( 'voice', $voice, $user_id, $page_id, $session_id);
    ...
    // Call the Text-to-Speech API
    $response = steven_bot_call_tts_api($api_key, $message, $voice);

    // Reset the model - REMOVED - Ver 2.0.6 - 2024 07 11
    // set_steven_bot_transients( 'model', $t_model, $user_id, $page_id, $session_id);
    // $kchat_settings['model'] = $t_model;
    // set_steven_bot_transients( 'voice', $t_voice, $user_id, $page_id, $session_id);
...
add_action('wp_ajax_nopriv_steven_bot_read_aloud', 'steven_bot_read_aloud');

// Set up the cron job to delete the audio file after 2 hours
function steven_bot_delete_audio_file_id( $file_id ) {
    ...
    set_transient('steven_bot_delete_audio_file_' . $file_id, $file_id, $timeFrameForDelete);
    ...
    if (!wp_next_scheduled('delete_audio_file', $file_id)) {
        wp_schedule_single_event($shorterTimeFrameForDelete, 'steven_bot_delete_audio_file', array($file_id));
    }
...
add_action( 'steven_bot_delete_audio_file', 'deleteAudioFile' );

// Delete old audio files - Ver 1.9.9
function steven_bot_cleanup_audio_directory() {

    global $steven_bot_plugin_dir_path;
    
    $audio_dir = $steven_bot_plugin_dir_path . 'audio/';
...
add_action('steven_bot_cleanup_audio_files', 'steven_bot_cleanup_audio_directory');
