<?php
/**
 * Steven-Bot - Azure OpenAI Assistants - Ver 2.2.6
 *
...
function add_an_azure_message($thread_id, $prompt, $context, $api_key, $file_id = null, $message_uuid = null) {
    ...
    if ( !empty($file_id) ) {
        ...
        $file_type = get_steven_bot_transients_files('steven_bot_assistant_file_type', $session_id, $file_id[0]);
            ...
            if (isset($message['attachments']) && is_array($message['attachments'])) {
                foreach ($message['attachments'] as $attachment) {
                    if (isset($attachment['file_id'])) {
                        ...
                        set_transient('steven_bot_delete_azure_uploaded_file_' . $file_id, $file_id, $timeFrameForDelete);
...
function chatbot_azure_retrieve_file_id( $user_id, $page_id ) {
    ...
    $file_types = get_steven_bot_transients_files('steven_bot_assistant_file_types', $session_id, $file_id);
    ...
    while (!empty($file_id)) {
        ...
        // delete_steven_bot_transients_files( 'steven_bot_assistant_file_id', $session_id, $counter);
