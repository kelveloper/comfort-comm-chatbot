<?php
/**
 * Steven-Bot - Settings - Support Pages - Overhauled in Ver 2.0.2.1
 *
...
function steven_bot_support_settings_init() {

    // Support settings tab - Ver 1.3.0
    register_setting('steven_bot_support', 'chatgpt_support_key', 'steven_bot_sanitize_text');

    add_settings_section(
        'steven_bot_support_section',
        'Support',
        'steven_bot_support_section_callback',
        'steven_bot_support'
    );

}
add_action('admin_init', 'steven_bot_support_settings_init');
...
function listDocumentationContents() {
    ...
    $documentationPath = $steven_bot_plugin_dir_path . '/documentation';
...
function steven_bot_support_section_callback() {
    ...
    if (validateDocumentation($dir, $file)) {
        $docLocation = $steven_bot_plugin_dir_path . 'documentation/' . $docLocation;
    } else {
        $docLocation = $steven_bot_plugin_dir_path . 'documentation/' . 'overview.md';
    }
    ...
    $basePath = $steven_bot_plugin_dir_path . 'documentation/';
    $basePath = "?page=steven-bot";
    if ($dir !== '') {
...
function adjustImagePath($url, $basePath) {
    ...
    if (strpos($url, 'http') !== 0) {
        ...
        if (count($basePathParts) > 1) {
            ...
            $url = site_url() . '/wp-content/plugins/steven-bot/documentation/' . $dir . '/' . $url;
        } else {
            // Check if the URL is a relative path and sanitize it
            $url = site_url() . '/wp-content/plugins/steven-bot/documentation/' . $url;
        }
