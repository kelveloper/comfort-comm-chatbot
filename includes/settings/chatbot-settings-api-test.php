<?php
/**
 * Steven-Bot - Settings - API/Model Test
 *
...
// Test API for status and errors - Ver 1.6.3 - Update Ver 2.2.1
 function kchat_test_api_status() {
    ...
    if ($chatbot_chatbot_ai_platform_choice == 'OpenAI') {
        update_option('steven_bot_api_status', 'API Error Type: Status Unknown');
        $api_key = esc_attr(get_option('steven_bot_api_key', 'NOT SET'));
        ...
        update_option('steven_bot_api_status', $updated_status);
...
function kchat_fetch_api_status($api_key, $model) {
    ...
    switch ($chatbot_ai_platform_choice) {
        ...
        case 'OpenAI':
            ...
            // Model and message for testing
            $model = esc_attr(get_option('steven_bot_model_choice', 'chatgpt-4o-latest'));
...
function chatgpt_option_updated($option_name, $old_value, $new_value) {
    ...
    if ($option_name !== 'chatgpt_model_choice') {
        return;
    } elseif ($option_name !== 'steven_bot_api_key') {
        return;
    }
    ...
    // FIXME Retrieve the current value of the steven_bot_api_status option
    $steven_bot_api_status = esc_attr(get_option('steven_bot_api_status', 'NOT SET'));
    // DIAG - Log the current value of the steven_bot_api_status option
    // back_trace( 'NOTICE', $steven_bot_api_status);
    
    // Check if the option updated is related to your plugin settings
    // if ($option_name === 'steven_bot_model_choice' || $option_name === 'steven_bot_api_key' || empty($steven_bot_api_status)) {
    if ($option_name === 'steven_bot_model_choice' || $option_name === 'steven_bot_api_key') {
        $api_key = esc_attr(get_option('steven_bot_api_key', 'NOT SET'));
        // Decrypt the API key - Ver 2.2.6
        $api_key = steven_bot_decrypt_api_key($api_key);
        ...
        update_option('steven_bot_api_status', $test_result);
...
// AJAX handler for manual API test button
function chatbot_ajax_test_api_status() {
    // Verify nonce
    if (!isset($_POST['chatbot_nonce']) || !wp_verify_nonce($_POST['chatbot_nonce'], 'chatbot_test_api')) {
    ...
    // Run the API test
    $result = kchat_test_api_status();

    wp_send_json_success($result);
}
add_action('wp_ajax_chatbot_test_api_status', 'chatbot_ajax_test_api_status');
