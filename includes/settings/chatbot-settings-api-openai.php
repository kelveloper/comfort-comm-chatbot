<?php
/**
 * Steven-Bot - Settings - API/ChatGPT Page
 *
...
function steven_bot_model_settings_section_callback($args) {
    ...
    <p style="background-color: #e0f7fa; padding: 10px;"><b>For an explanation of the API/ChatGPT Settings and additional documentation please click <a href="?page=steven-bot&tab=support&dir=api-chatgpt-settings&file=api-chatgpt-model-settings.md">here</a>.</b></p>
    <?php

}

function steven_bot_api_chatgpt_general_section_callback($args) {
...
function steven_bot_api_chatgpt_chat_section_callback($args) {
...
function steven_bot_api_chatgpt_image_section_callback($args) {
...
function steven_bot_api_chatgpt_voice_section_callback($args) {
...
function steven_bot_api_chatgpt_whisper_section_callback($args) {
...
function steven_bot_api_chatgpt_advanced_section_callback($args) {
...
function steven_bot_api_key_callback($args) {
    ...
    ?>
    <input type="password" id="steven_bot_api_key" name="steven_bot_api_key" value="<?php echo esc_attr( $api_key ); ?>" class="regular-text"  autocomplete="off">
    <?php
    ...
    if (is_string($models) && strpos($models, 'Error:') === 0) {
        ...
        <select id="steven_bot_model_choice" name="steven_bot_model_choice">
            <?php foreach ($models as $model): ?>
                <option value="<?php echo esc_attr($model['id']); ?>" <?php selected(esc_attr(get_option('steven_bot_model_choice')), $model['id']); ?>><?php echo esc_html($model['id']); ?></option>
            <?php endforeach; ?>
            ?>
        </select>
        <?php
...
function chatgpt_max_tokens_setting_callback($args) {
    ...
    // Get the saved steven_bot_max_tokens_setting or default to 500
    $max_tokens = esc_attr(get_option('steven_bot_max_tokens_setting', '500'));
    // Allow for a range of tokens between 100 and 10000 in 100-step increments - Ver 2.0.4
    ?>
    <select id="steven_bot_max_tokens_setting" name="steven_bot_max_tokens_setting">
        <?php
...
function steven_bot_conversation_context_callback($args) {

    // Get the value of the setting we've registered with register_setting()
    $steven_bot_conversation_context = esc_attr(get_option('steven_bot_conversation_context'));

    // Check if the option has been set, if not, use a default value
    if (empty($steven_bot_conversation_context)) {
        ...
        update_option('steven_bot_conversation_context', $steven_bot_conversation_context);
    }

    ?>
    <!-- Define the textarea field. -->
    <textarea id='steven_bot_conversation_context' name='steven_bot_conversation_context' rows='5' cols='50' maxlength='12500'><?php echo esc_html(stripslashes($steven_bot_conversation_context)); ?></textarea>
    <?php

}

// Set steven_bot_temperature - Ver 2.0.1
// https://platform.openai.com/docs/assistants/how-it-works/temperature
function steven_bot_temperature_callback($args) {

    $temperature = esc_attr(get_option('steven_bot_temperature', 0.50));
    ?>
    <select id="steven_bot_temperature" name="steven_bot_temperature">
        <?php
...
// Set steven_bot_top_p - Ver 2.0.1
// https://platform.openai.com/docs/assistants/how-it-works/top-p
function steven_bot_top_p_callback($args) {

    $top_p = esc_attr(get_option('steven_bot_top_p', 1.00));
    ?>
    <select id="steven_bot_top_p" name="steven_bot_top_p">
        <?php
...
function steven_bot_base_url_callback($args) {

    $steven_bot_base_url = esc_attr(get_option('steven_bot_base_url', 'https://api.openai.com/v1'));
    ?>
    <input type="text" id="steven_bot_base_url" name="steven_bot_base_url" value="<?php echo esc_attr( $steven_bot_base_url ); ?>" class="regular-text">
    <?php

}

// Timeout Settings Callback - Ver 1.8.8
function steven_bot_timeout_setting_callback($args) {

    // Get the saved steven_bot_timeout value or default to 240
    $timeout = esc_attr(get_option('steven_bot_timeout_setting', 240));

    // Allow for a range of tokens between 5 and 500 in 5-step increments - Ver 1.8.8
    ?>
    <select id="steven_bot_timeout_setting" name="steven_bot_timeout_setting">
        <?php
        for ($i=5; $i<=500; $i+=5) {
...
function steven_bot_voice_model_option_callback($args) {

    // https://platform.openai.com/docs/guides/voice-models
    // https://platform.openai.com/docs/models/tts

    // Get the saved steven_bot_model_choice value or default to "gpt-3.5-turbo"
    $voice_model_option = esc_attr(get_option('steven_bot_voice_model_option', 'tts-1-hd'));
    ...
    if (is_string($voice_models) && strpos($voice_models, 'Error:') === 0) {
        ...
        $voice_model_option = esc_attr(get_option('steven_bot_voice_model_option', 'tts-1-hd'));
        ?>
        <select id="steven_bot_voice_model_option" name="steven_bot_voice_model_option">
        ...
        <select id="steven_bot_voice_model_option" name="steven_bot_voice_model_option">
            <?php foreach ($voice_models as $voice_model): ?>
                <option value="<?php echo esc_attr($voice_model['id']); ?>" <?php selected(esc_attr(get_option('steven_bot_voice_model_option')), $voice_model['id']); ?>><?php echo esc_html($voice_model['id']); ?></option>
            <?php endforeach; ?>
        </select>
        <?php  
    }
...
function steven_bot_voice_option_callback($args) {

    // https://platform.openai.com/docs/guides/speech-to-text
    // Options include Alloy, Echo, Fable, Onyx, Nova, and Shimmer

    // Get the saved steven_bot_voice_options value or default to "Alloy"
    $voice_option = esc_attr(get_option('steven_bot_voice_option', 'alloy'));
    ?>
    <select id="steven_bot_voice_option" name="steven_bot_voice_option">
...
function steven_bot_audio_output_format_callback($args) {

    // https://platform.openai.com/docs/guides/text-to-speech
    // Options include mp3, opus, aac, flac, wav, and pcm

    // Get the saved steven_bot_voice_output_options value or default to "mp3"
    $audio_output_format = esc_attr(get_option('steven_bot_audio_output_format', 'mp3'));
    ?>
    <select id="steven_bot_audio_output_format" name="steven_bot_audio_output_format">
...
function steven_bot_image_model_option_callback($args) {

    // https://platform.openai.com/docs/guides/images
    // https://platform.openai.com/docs/models/dall-e

    // Get the saved steven_bot_model_option value or default to "dall-e-3"
    $image_model_option = esc_attr(get_option('steven_bot_image_model_option', 'dall-e-3'));
    ...
    if (is_string($image_models) && strpos($image_models, 'Error:') === 0) {
        ...
        $image_model_option = esc_attr(get_option('steven_bot_image_model_option', 'dall-e-3'));
        ?>
        <select id="steven_bot_image_model_option" name="steven_bot_image_model_option">
        ...
        <select id="steven_bot_image_model_option" name="steven_bot_image_model_option">
            <?php foreach ($image_models as $image_model): ?>
                <option value="<?php echo esc_attr($image_model['id']); ?>" <?php selected(esc_attr(get_option('steven_bot_image_model_option')), $image_model['id']); ?>><?php echo esc_html($image_model['id']); ?></option>
            <?php endforeach; ?>
        </select>
        <?php  
    }
...
function steven_bot_image_output_format_callback($args) {

    // https://platform.openai.com/docs/guides/images
    // Options include png

    // Get the saved steven_bot_image_output_format value or default to "png"
    $image_output_format = esc_attr(get_option('steven_bot_image_output_format', 'png'));
    ?>
    <select id="steven_bot_image_output_format" name="steven_bot_image_output_format">
...
function steven_bot_image_output_size_callback($args) {

    // https://platform.openai.com/docs/guides/images
    // The size of the generated images. Must be one of 256x256, 512x512, or 1024x1024 for dall-e-2.
    // Must be one of 1024x1024, 1792x1024, or 1024x1792 for dall-e-3 models.

    // Get the saved steven_bot_image_model_option value or default to "dall-e-3"
    $model = esc_attr(get_option('steven_bot_image_model_option', 'dall-e-3')); 

    // Get the saved steven_bot_image_output_size value or default to "1024x1024"
    $image_output_size = esc_attr(get_option('steven_bot_image_output_size', '1024x1024'));
    ...
    ?>
    <select id="steven_bot_image_output_size" name="steven_bot_image_output_size">
...
function steven_bot_image_output_quantity_callback($args) {
    ...
    $image_output_quantity = esc_attr(get_option('steven_bot_image_output_quantity', '1'));

    $model = esc_attr(get_option('steven_bot_image_model_option', 'dall-e-3'));
    
    // Display the options based on model selection
    ?>
        <select id="steven_bot_image_output_quantity" name="steven_bot_image_output_quantity">
...
function steven_bot_image_output_quality_callback($args) {

    // https://platform.openai.com/docs/guides/images
    // Options include standard

    // Get the saved steven_bot_image_output_quality value or default to "3"
    $image_output_quality = esc_attr(get_option('steven_bot_image_output_quality', 'standard'));
    ?>
    <select id="steven_bot_image_output_quality" name="steven_bot_image_output_quality">
...
function steven_bot_image_style_output_callback($args) {

    // https://platform.openai.com/docs/guides/images
    // Options include standard

    // Get the saved steven_bot_image_style_output value or default to "3"
    $image_style_output = esc_attr(get_option('steven_bot_image_style_output', 'vivid'));
    ?>
    <select id="steven_bot_image_style_output" name="steven_bot_image_style_output">
...
function steven_bot_whisper_model_option_callback($args) {
    
        // https://platform.openai.com/docs/models/whisper
        // Options include whisper-1
    
        // Get the saved steven_bot_whisper_model_option value or default to "whisper-1"
        $whisper_model_option = esc_attr(get_option('steven_bot_whisper_model_option', 'whisper-1'));
        ...
        if (is_string($whisper_models) && strpos($whisper_models, 'Error:') === 0) {
            ...
            $whisper_model_option = esc_attr(get_option('steven_bot_whisper_model_option', 'whisper-1'));
            ?>
            <select id="steven_bot_whisper_model_option" name="steven_bot_whisper_model_option">
            ...
            <select id="steven_bot_whisper_model_option" name="steven_bot_whisper_model_option">
                <?php foreach ($whisper_models as $whisper_model): ?>
                    <option value="<?php echo esc_attr($whisper_model['id']); ?>" <?php selected(esc_attr(get_option('steven_bot_whisper_model_option')), $whisper_model['id']); ?>><?php echo esc_html($whisper_model['id']); ?></option>
                <?php endforeach; ?>
            </select>
            <?php  
        }
...
function steven_bot_whisper_response_format_callback($args) {
    
        // https://platform.openai.com/docs/models/whisper
        // Options include mp3
    
        // Get the saved steven_bot_whisper_response_format value or default to "text"
        $whisper_response_format = esc_attr(get_option('steven_bot_whisper_response_format', 'text'));
        ?>
        <select id="steven_bot_whisper_response_format" name="steven_bot_whisper_response_format">
