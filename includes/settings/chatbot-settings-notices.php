...
add_action('admin_notices', 'steven_bot_general_admin_notice');
...
function display_option_value_admin_notice() {
    ...
    if ($steven_bot_suppress_notices == 'On') {
        return;
    }

    $kn_results = esc_attr(get_option('steven_bot_kn_results'));
    if ($kn_results) {
    ...
    $kn_dismissed = esc_attr(get_option('steven_bot_kn_dismissed'));
...
function dismiss_chatgpt_notice() {
    if (isset($_GET['dismiss_chatgpt_notice'])) {
        delete_option('steven_bot_kn_results');
    }
    if (isset($_GET['dismiss_kn_status_notice'])) {
        update_option('steven_bot_kn_dismissed', '1');
        // DIAG - Diagnostics - Ver 2.0.4
        // back_trace( 'NOTICE' , 'steven_bot_kn_dismissed updated to 1');
    }
}
