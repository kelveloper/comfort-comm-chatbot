...
function steven_bot_reporting_settings_init() {

    // Register settings for Reporting
    register_setting('steven_bot_reporting', 'steven_bot_reporting_period', 'steven_bot_sanitize_text');
    register_setting('steven_bot_reporting', 'steven_bot_enable_conversation_logging', 'steven_bot_sanitize_checkbox');
    register_setting('steven_bot_reporting', 'steven_bot_conversation_log_days_to_keep', 'steven_bot_sanitize_text');

    // Reporting Overview Section
    add_settings_section(
        'steven_bot_reporting_overview_section',
        'Reporting Overview',
        'steven_bot_reporting_overview_section_callback',
        'steven_bot_reporting_overview'
    );

    // Reporting Settings Section
    add_settings_section(
        'steven_bot_reporting_section',
        'Reporting Settings',
        'steven_bot_reporting_section_callback',
        'steven_bot_reporting'
    );

    // Reporting Settings Field - Reporting Period
    add_settings_field(
        'steven_bot_reporting_period',
        'Reporting Period',
        'steven_bot_reporting_period_callback',
        'steven_bot_reporting',
        'steven_bot_reporting_section'
    );
    ...
    add_settings_field(
        'steven_bot_enable_conversation_logging',
        'Enable Conversation Logging',
        'steven_bot_enable_conversation_logging_callback',
        'steven_bot_reporting',
        'steven_bot_reporting_section'
    );
    ...
    add_settings_field(
        'steven_bot_conversation_log_days_to_keep',
        'Conversation Log Days to Keep',
        'steven_bot_conversation_log_days_to_keep_callback',
        'steven_bot_reporting',
        'steven_bot_reporting_section'
    );
    ...
    add_settings_section(
        'steven_bot_conversation_reporting_section',
        'Conversation Data',
        'steven_bot_conversation_reporting_section_callback',
        'steven_bot_conversation_reporting'
    );

    add_settings_field(
        'steven_bot_conversation_reporting_field',
        'Conversation Data',
        'steven_bot_conversation_reporting_callback',
        'steven_bot_reporting',
        'steven_bot_conversation_reporting_section'
    );
    ...
    add_settings_section(
        'steven_bot_interaction_reporting_section',
        'Interaction Data',
        'steven_bot_interaction_reporting_section_callback',
        'steven_bot_interaction_reporting'
    );

    add_settings_field(
        'steven_bot_interaction_reporting_field',
        'Interaction Data',
        'steven_bot_interaction_reporting_callback',
        'steven_bot_reporting',
        'steven_bot_interaction_reporting_section'
    );
    ...
    // // Token Data Section
    add_settings_section(
        'steven_bot_token_reporting_section',
        'Token Data',
        'steven_bot_token_reporting_section_callback',
        'steven_bot_token_reporting'
    );

    add_settings_field(
        'steven_bot_token_reporting_field',
        'Token Data',
        'steven_bot_token_reporting_callback',
        'steven_bot_reporting',
        'steven_bot_token_reporting_section'
    );
    ...
    add_settings_section(
        'steven_bot_gap_analysis_section',
        'Gap Analysis',
        'steven_bot_gap_analysis_section_callback',
        'steven_bot_gap_analysis'
    );

    add_settings_field(
        'steven_bot_gap_analysis_field',
        '',
        'steven_bot_gap_analysis_callback',
        'steven_bot_gap_analysis',
        'steven_bot_gap_analysis_section'
    );
    ...
    add_settings_section(
        'steven_bot_learning_dashboard_section',
        'Learning Dashboard',
        'steven_bot_learning_dashboard_section_callback',
        'steven_bot_learning_dashboard'
    );

    add_settings_field(
        'steven_bot_learning_dashboard_field',
        '',
        'steven_bot_learning_dashboard_callback',
        'steven_bot_learning_dashboard',
        'steven_bot_learning_dashboard_section'
    );

}
add_action('admin_init', 'steven_bot_reporting_settings_init');

// Reporting section callback - Ver 1.6.3
function steven_bot_reporting_overview_section_callback($args) {
    // Get CSAT stats
    $csat_stats = steven_bot_get_csat_stats();
    ...
    <div>
        ...
        <p>Please review the section <b>Conversation Logging Overview</b> on the <a href="?page=steven-bot&tab=support&dir=support&file=conversation-logging-and-history.md">Support</a> tab of this plugin for more details.</p>
        <p><b><i>Don't forget to click </i><code>Save Settings</code><i> to save any changes your might make.</i></b></p>
        <p style="background-color: #e0f7fa; padding: 10px;"><b>For an explanation on how to use the Reporting and additional documentation please click <a href="?page=steven-bot&tab=support&dir=reporting&file=reporting.md">here</a>.</b></p>
    </div>
    <?php
}

function steven_bot_reporting_section_callback($args) {
    ?>
    <div>
...
function steven_bot_conversation_reporting_section_callback($args) {
    ?>
    <div>
        <p>Conversation items stored in your DB total <b><?php echo steven_bot_count_conversations(); ?></b> rows (includes both Visitor and User input and chatbot responses).</p>
        <p>Conversation items stored take up <b><?php echo steven_bot_size_conversations(); ?> MB</b> in your database.</p>
        <p>Use the button (below) to retrieve the conversation data and download as a CSV file.</p>
        <?php
            if (is_admin()) {
                $header = " ";
                $header .= '<a class="button button-primary" href="' . esc_url(admin_url('admin-post.php?action=steven_bot_download_conversation_data')) . '">Download Conversation Data</a>';
                echo $header;
            }
        ?>
    </div>
...
function steven_bot_interaction_reporting_section_callback($args) {
    ?>
    <div>
        ...
        <p><?php echo steven_bot_interactions_table() ?></p>
        <p>Use the button (below) to retrieve the interactions data and download as a CSV file.</p>
        <?php
            if (is_admin()) {
                $header = " ";
                $header .= '<a class="button button-primary" href="' . esc_url(admin_url('admin-post.php?action=steven_bot_download_interactions_data')) . '">Download Interaction Data</a>';
                echo $header;
            }
        ?>
    </div>
...
function steven_bot_token_reporting_section_callback($args) {
    ?>
    <div>
        <p><?php echo steven_bot_total_tokens() ?></p>
        <p>Use the button (below) to retrieve the interactions data and download as a CSV file.</p>
        <?php
            if (is_admin()) {
                $header = " ";
                $header .= '<a class="button button-primary" href="' . esc_url(admin_url('admin-post.php?action=steven_bot_download_token_usage_data')) . '">Download Token Usage Data</a>';
                echo $header;
            }
        ?>
    </div>
...
function steven_bot_reporting_settings_callback($args){
    ?>
    <div>
...
function steven_bot_reporting_period_callback($args) {
    // Get the saved steven_bot_reporting_period value or default to "Daily"
    $output_choice = esc_attr(get_option('steven_bot_reporting_period', 'Daily'));
    // DIAG - Log the output choice
    // back_trace( 'NOTICE', 'steven_bot_reporting_period' . $output_choice);
    ?>
    <select id="steven_bot_reporting_period" name="steven_bot_reporting_period">
...
function  steven_bot_enable_conversation_logging_callback($args) {
    ...
    ?>
    <select id="steven_bot_enable_conversation_logging" name="steven_bot_enable_conversation_logging">
...
function steven_bot_conversation_log_days_to_keep_callback($args) {
    // Get the saved steven_bot_conversation_log_days_to_keep value or default to "30"
    $output_choice = esc_attr(get_option('steven_bot_conversation_log_days_to_keep', '30'));
    // DIAG - Log the output choice
    // back_trace( 'NOTICE', 'steven_bot_conversation_log_days_to_keep' . $output_choice);
    ?>
    <select id="steven_bot_conversation_log_days_to_keep" name="steven_bot_conversation_log_days_to_keep">
...
function steven_bot_simple_chart_shortcode_function( $atts ) {
    ...
    if (!extension_loaded('gd')) {
        ...
        steven_bot_general_admin_notice('Chatbot requires the GD Library to function correctly, but it is not installed or enabled on your server. Please install or enable the GD Library.');
        // DIAG - Log the output choice
    ...
    $reporting_period = esc_attr(get_option('steven_bot_reporting_period'));
    ...
    wp_schedule_single_event(time() + 60, 'steven_bot_delete_chart', array($img_path)); // 60 seconds delay

    return '<img src="' . $img_url . '" alt="Bar Chart">';
}
// TEMPORARILY REMOVED AS SOME USERS ARE EXPERIENCING ISSUES WITH THE CHARTS - Ver 1.7.8
// Add shortcode
// add_shortcode('steven_bot_simple_chart', 'steven_bot_simple_chart_shortcode_function');
// add_shortcode('chatbot_simple_chart', 'steven_bot_simple_chart_shortcode_function');


// Clean up ../image subdirectory - Ver 1.6.3
function steven_bot_delete_chart() {
...
add_action('steven_bot_delete_chart', 'steven_bot_delete_chart');

// Return Interactions data in a table - Ver 1.7.8
// Updated Ver 2.4.8: Uses Supabase only
function steven_bot_interactions_table() {
...
// Updated Ver 2.4.8: Uses Supabase only
function steven_bot_count_conversations() {
...
// Updated Ver 2.4.8: Uses Supabase only
function steven_bot_size_conversations() {
...
// Updated Ver 2.4.8: Uses Supabase only
function steven_bot_total_tokens() {
...
function steven_bot_download_interactions_data() {

    // Export data from the steven_bot_interactions table to a csv file
    steven_bot_export_data('steven_bot_interactions', 'Chatbot-ChatGPT-Interactions');

}

function steven_bot_download_conversation_data() {

    // Export data from the steven_bot_conversation_log table to a csv file
    steven_bot_export_data('steven_bot_conversation_log', 'Chatbot-ChatGPT-Conversation Logs');
    
}

function steven_bot_download_token_usage_data() {

    // Export data from the steven_bot_conversation_log table to a csv file
    steven_bot_export_data('steven_bot_conversation_log', 'Chatbot-ChatGPT-Token Usage');

}

// Download the conversation data - Ver 1.7.6
// Updated Ver 2.4.8: Uses Supabase for data export
function steven_bot_export_data( $t_table_name, $t_file_name ) {
    ...
    if ($t_table_name === 'steven_bot_conversation_log') {
    ...
    } elseif ($t_table_name === 'steven_bot_interactions') {
    ...
    if (empty($results)) {
        $message = __( 'No data in the file. Please enable conversation and interaction logging if currently off.', 'steven-bot' );
        set_transient('steven_bot_admin_error', $message, 60); // Expires in 60 seconds
        wp_safe_redirect(admin_url('options-general.php?page=steven-bot&tab=reporting')); // Redirect to your settings page
        exit;
    }
    ...
    $results_dir_path = $steven_bot_plugin_dir_path . 'results/';
    ...
    if ($file === false) {
        $message = __( 'Error opening file for writing. Please try again.', 'steven-bot' );
        set_transient('steven_bot_admin_error', $message, 60); // Expires in 60 seconds
        wp_safe_redirect(admin_url('options-general.php?page=steven-bot&tab=reporting')); // Redirect to your settings page
        exit;
    }
    ...
    if (isset($results[0]) && is_array($results[0])) {
        ...
        $message = __( 'Chatbot No data in the file. Please enable conversation logging if currently off.', 'steven-bot' );
    ...
    if (!file_exists($results_csv_file)) {
        ...
        $message = sprintf( __( 'File not found: %s', 'steven-bot' ), $results_csv_file );
        // printf( '<div class="%1$s"><p>%2$s</p></div>', esc_attr( $class ), esc_html( $message ) );
        steven_bot_general_admin_notice($message);
        return;
    }
    ...
    if ($csv_data === false) {
        $class = 'notice notice-error';
        $message = __( 'Error reading file', 'steven-bot' );
        steven_bot_general_admin_notice($message);
        return;
    }
...
add_action('admin_post_steven_bot_download_conversation_data', 'steven_bot_download_conversation_data');
add_action('admin_post_steven_bot_download_interactions_data', 'steven_bot_download_interactions_data');
add_action('admin_post_steven_bot_download_token_usage_data', 'steven_bot_download_token_usage_data');

// Gap Analysis Section Callback - Ver 2.4.2
function steven_bot_gap_analysis_section_callback($args) {
    ?>
...
function steven_bot_gap_analysis_callback($selected_period = null) {
...
function steven_bot_learning_dashboard_section_callback($args) {
    ?>
...
function steven_bot_learning_dashboard_callback() {
...
function steven_bot_admin_notice() {
    $message = get_transient('steven_bot_admin_error');
    if (!empty($message)) {
        printf('<div class="%1$s"><p><b>Chatbot: </b>%2$s</p></div>', 'notice notice-error is-dismissible', $message);
        delete_transient('steven_bot_admin_error'); // Clear the transient after displaying the message
    }
}
add_action('admin_notices', 'steven_bot_admin_notice');
...
function chatbot_register_data_retention_settings() {
    register_setting('steven_bot_reporting', 'chatbot_retention_conversations', 'steven_bot_sanitize_text');
    register_setting('steven_bot_reporting', 'chatbot_retention_interactions', 'steven_bot_sanitize_text');
    register_setting('steven_bot_reporting', 'chatbot_retention_gap_questions', 'steven_bot_sanitize_text');
    register_setting('steven_bot_reporting', 'chatbot_retention_gap_clusters', 'steven_bot_sanitize_text');
