<?php
/**
 * Steven-Bot - Chatbot WIDGET ENDPOINT - Ver 2.1.3 - Updated Ver 2.2.7 - 2025-03-21
 *
...
if ($chatbot_ai_platform_choice === 'Azure OpenAI') {
    ...
    $chatbot_enable_remote_widget = esc_attr(get_option('steven_bot_enable_remote_widget', 'No'));
}
...
if ($chatbot_ai_platform_choice === 'Azure OpenAI') {
    ...
    $allowed_domains_string = esc_attr(get_option('steven_bot_allowed_remote_domains', ''));
}
...
if (is_user_logged_in()) {
    ...
    $kchat_settings['steven_bot_message_limit_setting_period'] = esc_attr(get_option('steven_bot_user_message_limit_period_setting', 'Lifetime'));
    ...
    $kchat_settings['steven_bot_message_limit_setting_period'] = esc_attr(get_option('steven_bot_visitor_message_limit_period_setting', 'Lifetime'));
    $kchat_settings['steven_bot_display_message_count'] = esc_attr(get_option('steven_bot_display_message_count', 'No'));
}
...
$kchat_settings = array_merge($kchat_settings,array(
    'steven-bot-version' => esc_attr($steven_bot_plugin_version),
    'plugins_url' => esc_url($steven_bot_plugin_dir_url),
...
// $steven_bot_widget_width = esc_attr(get_option('steven_bot_widget_width', '500'));
// $steven_bot_widget_height = '45hv';
...
<body>
    ...
    <script type="text/javascript">
        ...
        localStorage.setItem('steven_bot_start_status_new_visitor', 'open');
