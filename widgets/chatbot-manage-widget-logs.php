<?php
/**
 * Steven-Bot - Manage Widget Logs - Ver 2.1.3
 *
...
function chatbot_manage_widget_logs() {
    ...
    $chatbot_logs_dir = $steven_bot_plugin_dir_path . 'widget-logs/';
...
function handle_widget_log_actions() {
    ...
    switch ($action) {
        ...
        case 'delete_widget_log':
            ...
            if (file_exists($file_path)) {
                unlink($file_path);
                wp_redirect(admin_url('admin.php?page=steven-bot&tab=tools')); // Redirect to plugin page
                exit;
            } else {
                wp_die('File not found');
            }
