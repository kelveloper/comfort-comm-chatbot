 ...
 * along with Steven-Bot. If not, see https://www.gnu.org/licenses/gpl-3.0.html.
 * 
 */
...
$steven_bot_plugin_dir_path = plugin_dir_path( __FILE__ );

// Plugin directory URL
global $steven_bot_plugin_dir_url;
$steven_bot_plugin_dir_url = plugins_url( '/', __FILE__ );
...
function kognetiks_get_unique_id() {
    if (isset($_COOKIE['steven_unique_id'])) {
        // error_log('[Chatbot] [steven-bot.php] Unique ID found: ' . $_COOKIE['steven_unique_id']);
        return sanitize_text_field($_COOKIE['steven_unique_id']);
    }
    // error_log('[Chatbot] [steven-bot.php] Unique ID not found');
    return null;
}

// Fetch the User ID - Updated Ver 2.0.6 - 2024 07 11
$user_id = get_current_user_id();
// Fetch the chatbot cookie
...
if (!function_exists('steven_bot_appearance_icon_path')) {
    function steven_bot_appearance_icon_path($icon_type) {
...
require_once plugin_dir_path(__FILE__) . 'includes/chatbot-call-gemini-api.php';            // Gemini API - Ver 2.3.7
require_once plugin_dir_path(__FILE__) . 'includes/chatbot-call-kognetiks-api-mc.php';      // Steven-Bot - Markov Chain API - Ver 2.1.6
require_once plugin_dir_path(__FILE__) . 'includes/chatbot-call-kognetiks-api-tm.php';      // Steven-Bot - Transformer Model API - Ver 2.2.0
...
require_once plugin_dir_path(__FILE__) . 'includes/chatbot-call-openai-api-kflow.php';      // Steven-Bot - Flow API - Ver 1.9.5
...
require_once plugin_dir_path(__FILE__) . 'includes/settings/chatbot-settings-api-gemini.php';              // Gemini
...
require_once plugin_dir_path(__FILE__) . 'includes/dashboard/steven-bot-dashboard-widget.php';     // Dashboard Widget - Ver 2.2.7
...
require_once plugin_dir_path(__FILE__) . 'includes/utilities/chatbot-sanitization.php';               // Sanitization callbacks - Ver 2.5.0
...
add_action('admin_init', function() {
    if (
        isset($_POST['steven_bot_analytics_action']) &&
        $_POST['steven_bot_analytics_action'] === 'period_filter' &&
        isset($_POST['steven_bot_analytics_period_filter_nonce']) &&
        wp_verify_nonce(
            sanitize_text_field(wp_unslash($_POST['steven_bot_analytics_period_filter_nonce'])),
            'steven_bot_analytics_period_filter_action'
        )
    ) {
        ...
        $selected_period = isset($_POST['steven_bot_analytics_period_filter'])
            ? sanitize_text_field(wp_unslash($_POST['steven_bot_analytics_period_filter']))
            : 'Today';
        // Store in a transient or option, or pass as needed
        set_transient('steven_bot_selected_period', $selected_period, 60*5);
        wp_redirect(admin_url('admin.php?page=steven-bot&tab=analytics'));
        exit;
    }
});
...
if (!esc_attr(get_option('steven_bot_upgraded'))) {
    steven_bot_upgrade();
    update_option('steven_bot_upgraded', 'Yes');
}
...
switch ($chatbot_ai_platform_choice) {
    ...
    case 'OpenAI':
        ...
        $steven_bot_api_enabled = 'Yes';
        update_option('steven_bot_api_enabled', 'Yes');
        ...
        $chatbot_gemini_api_enabled = 'No';
        update_option('chatbot_gemini_api_enabled', 'No');
        ...
        if (esc_attr(get_option('steven_bot_model_choice')) === null) {
            $model = 'gpt-4-1106-preview';
            update_option('steven_bot_model_choice', $model);
            // DIAG - Diagnostics
        ...
        if (esc_attr(get_option('steven_bot_voice_option')) === null) {
            $voice = 'alloy';
            update_option('steven_bot_voice_option', $voice);
            // DIAG - Diagnostics
    ...
    case 'Azure OpenAI':
        ...
        $steven_bot_api_enabled = 'No';
        update_option('steven_bot_api_enabled', 'No');
        ...
        update_option('steven_bot_read_aloud_option', 'no');
        // Voice choice - Ver 1.9.5
    ...
    case 'NVIDIA':
        ...
        update_option('steven_bot_allow_file_uploads', 'No');
        // Disable MP3 Uploads - Ver 2.2.1
        update_option('steven_bot_allow_mp3_uploads', 'No');
    ...
    case 'Gemini':
        ...
        update_option('chatbot_ai_platform_choice', 'Gemini');
        ...
        $chatbot_gemini_api_enabled = 'Yes';
        update_option('chatbot_gemini_api_enabled', 'Yes');
        ...
        update_option('steven_bot_read_aloud_option', 'no');
        // Disable File Uploads - Ver 2.3.7
        update_option('steven_bot_allow_file_uploads', 'No');
        // Disable MP3 Uploads - Ver 2.3.7
        update_option('steven_bot_allow_mp3_uploads', 'No');
...
$steven_bot_enable_custom_buttons = esc_attr(get_option('steven_bot_enable_custom_buttons', 'Off'));

// Allow file uploads on/off setting can be found on the Settings tab - Ver 1.7.6
global $steven_bot_allow_file_uploads;
$chatbot_ai_platform_choice = esc_attr(get_option('chatbot_ai_platform_choice', 'OpenAI'));
if ($chatbot_ai_platform_choice == 'OpenAI') {
    $steven_bot_allow_file_uploads = esc_attr(get_option('steven_bot_allow_file_uploads', 'No'));
    // DIAG - Diagnostics - Ver 2.2.6
    // back_trace( 'NOTICE', '$chatbot_ai_platform_choice: ' . $chatbot_ai_platform_choice);
    // back_trace( 'NOTICE', '$steven_bot_allow_file_uploads: ' . $steven_bot_allow_file_uploads);
} elseif ($chatbot_ai_platform_choice == 'Azure OpenAI') {
    $steven_bot_allow_file_uploads = esc_attr(get_option('chatbot_azure_allow_file_uploads', 'No'));
    // DIAG - Diagnostics - Ver 2.2.6
    // back_trace( 'NOTICE', '$chatbot_ai_platform_choice: ' . $chatbot_ai_platform_choice);
    // back_trace( 'NOTICE', '$steven_bot_allow_file_uploads: ' . $steven_bot_allow_file_uploads);
} else {
...
global $steven_bot_suppress_learnings;
$steven_bot_suppress_learnings = esc_attr(get_option('steven_bot_suppress_learnings', 'Random'));

// Context History - Ver 1.6.1
$context_history = [];

function steven_bot_enqueue_admin_scripts() {

    global $steven_bot_plugin_version;

    wp_enqueue_script('jquery'); // Ensure jQuery is enqueued
    wp_enqueue_script('steven_bot_admin', plugins_url('assets/js/steven-bot-admin.js', __FILE__), array('jquery'), $steven_bot_plugin_version, true);

}
add_action('admin_enqueue_scripts', 'steven_bot_enqueue_admin_scripts');

// Activation, deactivation, and uninstall functions
register_activation_hook(__FILE__, 'steven_bot_activate');
register_activation_hook(__FILE__, 'create_steven_bot_assistants_table');
register_activation_hook(__FILE__, 'create_chatbot_azure_assistants_table');
register_deactivation_hook(__FILE__, 'steven_bot_deactivate');
register_uninstall_hook(__FILE__, 'steven_bot_uninstall');
add_action('upgrader_process_complete', 'steven_bot_upgrade_completed', 10, 2);

// Enqueue plugin scripts and styles
function steven_bot_enqueue_scripts() {
    ...
    global $steven_bot_display_style;

    // Enqueue the styles
    wp_enqueue_style('dashicons');
    wp_enqueue_style('steven-bot-css', plugins_url('assets/css/steven-bot.css', __FILE__), array(), $steven_bot_plugin_version, 'all');

    // Enqueue the scripts
    wp_enqueue_script('jquery');
    wp_enqueue_script('steven-bot-js', plugins_url('assets/js/steven-bot.js', __FILE__), array('jquery'), $steven_bot_plugin_version, true);
    ...
    wp_enqueue_script( 'dompurify', plugin_dir_url(__FILE__) . 'assets/js/purify.min.js', array(), $steven_bot_plugin_version, true );
    ...
    if (is_user_logged_in()) {
        ...
        $kchat_settings['steven_bot_message_limit_setting'] = esc_attr(get_option('steven_bot_user_message_limit_setting', '999'));
        $kchat_settings['steven_bot_message_limit_period_setting'] = esc_attr(get_option('steven_bot_user_message_limit_period_setting', 'Lifetime'));
        $kchat_settings['steven_bot_display_message_count'] = esc_attr(get_option('steven_bot_display_message_count', 'No'));
    } else {
        // back_trace( 'NOTICE', 'User is NOT logged in');
        $kchat_settings['steven_bot_message_limit_setting'] = esc_attr(get_option('steven_bot_visitor_message_limit_setting', '999'));
        $kchat_settings['steven_bot_message_limit_period_setting'] = esc_attr(get_option('steven_bot_visitor_message_limit_period_setting', 'Lifetime'));
        $kchat_settings['steven_bot_display_message_count'] = esc_attr(get_option('steven_bot_display_message_count', 'No'));
    }

    // Localize the data for the chatbot - Ver 2.1.1.1
    $kchat_settings = array_merge($kchat_settings, array(
        'steven_bot_display_style' => $steven_bot_display_style,
        'steven_bot_version' => $steven_bot_plugin_version,
        'plugins_url' => $steven_bot_plugin_dir_url,
    ...
    wp_enqueue_script('steven-bot-js', plugins_url('assets/js/steven-bot.js', __FILE__), array(), $steven_bot_plugin_version, true);

    // Add the inline script to define kchat_settings before the main script runs - CHANGED THE LOAD ORDER IN VER 2.1.2
    wp_add_inline_script('steven-bot-js', 'if (typeof kchat_settings === "undefined") { var kchat_settings = ' . $kchat_settings_json . '; } else { kchat_settings = ' . $kchat_settings_json . '; }', 'before');
    // FIXME - REMOVED IN V2.1.2
    // wp_add_inline_script('steven-bot-local', 'if (typeof kchat_settings === "undefined") { var kchat_settings = ' . $kchat_settings_json . '; } else { kchat_settings = ' . $kchat_settings_json . '; }', 'before');

}
add_action('wp_enqueue_scripts', 'steven_bot_enqueue_scripts');
...
function enqueue_mathjax_with_custom_config() {
    ...
    if (esc_attr(get_option('steven_bot_enable_mathjax','No')) === 'Yes') {
        ...
        // wp_enqueue_script( 'mathjax', plugin_dir_url(__FILE__) . 'assets/js/tex-chtml.js', array(), $steven_bot_plugin_version, true );
        wp_enqueue_script( 'mathjax', $steven_bot_plugin_dir_url . 'assets/js/tex-mml-chtml.js', array(), $steven_bot_plugin_version, true );
...
if (!wp_next_scheduled('steven_bot_cleanup_event')) {
    wp_schedule_event(time(), 'daily', 'steven_bot_cleanup_event');
}
add_action('steven_bot_cleanup_event', 'clean_specific_expired_transients');

// Schedule Conversation Log Cleanup - Ver 1.6.7
if (!wp_next_scheduled('steven_bot_conversation_log_cleanup_event')) {
    wp_schedule_event(time(), 'daily', 'steven_bot_conversation_log_cleanup_event');
}
add_action('steven_bot_conversation_log_cleanup_event', 'steven_bot_conversation_log_cleanup');
...
if (!wp_next_scheduled('steven_bot_cleanup_transcript_files')) {
    wp_schedule_event(time(), 'hourly', 'steven_bot_cleanup_transcript_files');
}

// Deactivate old hooks - Ver 2.0.1
if(esc_attr(get_option('steven_bot_cleanup_old_hooks') !== 'Completed')) {
    // Deactivate old hooks - Ver 2.0.1
    wp_clear_scheduled_hook('steven_bot_cleanup_transcripts');
    // Then update the option
    update_option('steven_bot_cleanup_old_hooks', 'Completed');
}

// Schedule the audio file cleanup event if it's not already scheduled - Ver 1.9.9
// Schedule the cleanup event if it's not already scheduled
if (!wp_next_scheduled('steven_bot_cleanup_audio_files')) {
    wp_schedule_event(time(), 'hourly', 'steven_bot_cleanup_audio_files');
}

// Schedule the upload file cleanup event if it's not already scheduled - Ver 1.9.9
// Schedule the cleanup event if it's not already scheduled
if (!wp_next_scheduled('steven_bot_cleanup_upload_files')) {
    wp_schedule_event(time(), 'hourly', 'steven_bot_cleanup_upload_files');
}

// Schedule the download file cleanup event if it's not already scheduled - Ver 2.0.3
// Schedule the cleanup event if it's not already scheduled
if (!wp_next_scheduled('steven_bot_cleanup_download_files')) {
    wp_schedule_event(time(), 'hourly', 'steven_bot_cleanup_download_files');
}

// Add the Opean AI Assistant table to the database - Ver 2.0.4
// REMOVED Ver 2.2.7 and MOVED to the activation hook
// create_steven_bot_assistants_table();
...
function steven_bot_enqueue_message($user_id, $page_id, $session_id, $assistant_id, $message, $client_message_id = null) {
...
function steven_bot_dequeue_message($user_id, $page_id, $session_id, $assistant_id) {
    $queue_key = 'chatbot_message_queue_' . wp_hash($assistant_id . '|' . $user_id . '|' . $page_id . '|' . $session_id);
...
function steven_bot_get_queue_status($user_id, $page_id, $session_id, $assistant_id) {
    $queue_key = 'chatbot_message_queue_' . wp_hash($assistant_id . '|' . $user_id . '|' . $page_id . '|' . $session_id);
    $queue = get_transient($queue_key);
...
function steven_bot_process_queue($user_id, $page_id, $session_id, $assistant_id) {
    ...
    $queue_status = steven_bot_get_queue_status($user_id, $page_id, $session_id, $assistant_id);
    ...
    $message_data = steven_bot_dequeue_message($user_id, $page_id, $session_id, $assistant_id);
    if (!$message_data) {
        return false;
    }
    ...
    $response = steven_bot_process_queued_message($message_data);
    ...
    steven_bot_process_queue($user_id, $page_id, $session_id, $assistant_id);
...
function steven_bot_get_queue_status_ajax() {
...
function steven_bot_process_queued_message($message_data) {
    ...
    global $thread_id;
    global $assistant_id;
    global $steven_bot_assistant_alias;
    global $kchat_settings;
    global $additional_instructions;
    global $model;
    global $voice;
    ...
    switch ($chatbot_ai_platform_choice) {
        case 'OpenAI':
            ...
            $model = esc_attr(get_option('steven_bot_model_choice', 'gpt-4-1106-preview'));
            break;
        case 'Azure OpenAI':
    ...
    $kchat_settings['steven_bot_display_style'] = get_steven_bot_transients( 'display_style', $user_id, $page_id, $session_id);
    $kchat_settings['steven_bot_assistant_alias'] = get_steven_bot_transients( 'assistant_alias', $user_id, $page_id, $session_id);
    $kchat_settings['assistant_id'] = get_steven_bot_transients( 'assistant_id', $user_id, $page_id, $session_id);
    $kchat_settings['thread_id'] = get_steven_bot_transients( 'thread_id', $user_id, $page_id, $session_id);
    $kchat_settings['steven_bot_model'] = get_steven_bot_transients( 'model', $user_id, $page_id, $session_id);
    $kchat_settings['model'] = $kchat_settings['steven_bot_model'] ?: $model;
    $kchat_settings['steven_bot_voice_option'] = get_steven_bot_transients( 'voice', $user_id, $page_id, $session_id);
    $kchat_settings['additional_instructions'] = get_steven_bot_transients( 'additional_instructions', $user_id, $page_id, $session_id);
    
    $voice = $kchat_settings['steven_bot_voice_option'];
    $model = $kchat_settings['model'] ?: $model;
    $additional_instructions = $kchat_settings['additional_instructions'];
    $steven_bot_assistant_alias = $kchat_settings['steven_bot_assistant_alias'];
    ...
    if ($steven_bot_assistant_alias == 'original') {

        $use_assistant_id = 'No';
        // DIAG - Diagnostics - Ver 2.3.6
        // back_trace( 'NOTICE' , 'Using Original ChatGPT - $steven_bot_assistant_alias: ' . $steven_bot_assistant_alias);

    } elseif ($steven_bot_assistant_alias == 'primary') {
        ...
        if (empty($assistant_id) || $assistant_id == "Please provide the Assistant Id.") {
            ...
            $steven_bot_assistant_alias = 'original';
            $use_assistant_id = 'No';
    ...
    } elseif ($steven_bot_assistant_alias == 'alternate') {

        $assistant_id = esc_attr(get_option('steven_bot_assistant_id_alternate'));
        $use_assistant_id = 'Yes';
        ...
        $steven_bot_assistant_alias = $assistant_id; // Belt & Suspenders
        $use_assistant_id = 'Yes';
        ...
        // Check both $steven_bot_assistant_alias and $assistant_id - Ver 2.3.6
        if (!empty($steven_bot_assistant_alias) && (str_starts_with($steven_bot_assistant_alias, 'asst_') || str_starts_with($steven_bot_assistant_alias, 'ag:') || str_starts_with($steven_bot_assistant_alias, 'websearch'))) {
            
            // DIAG - Diagnostics - Ver 2.3.6
            // back_trace( 'NOTICE', 'Using GPT Assistant ID from alias: ' . $steven_bot_assistant_alias);

            // Override the $assistant_id with the GPT Assistant ID
            $assistant_id = $steven_bot_assistant_alias;
            $use_assistant_id = 'Yes';
            ...
            // Check $assistant_id directly if $steven_bot_assistant_alias is empty - Ver 2.3.6
            // DIAG - Diagnostics - Ver 2.3.6
            // back_trace( 'NOTICE', 'Using GPT Assistant ID from assistant_id: ' . $assistant_id);

            // Set the alias to match the assistant_id
            $steven_bot_assistant_alias = $assistant_id;
            $use_assistant_id = 'Yes';
            ...
            // DIAG - Diagnostics - Ver 2.3.6
            // back_trace( 'NOTICE', 'Using ChatGPT API: ' . $steven_bot_assistant_alias);
    ...
    // DIAG - Diagnostics - Ver 2.3.6
    // back_trace( 'NOTICE', 'Processing queued message - Platform: ' . $chatbot_ai_platform_choice . ', use_assistant_id: ' . $use_assistant_id);
    
    // Check if we should use assistant_id or regular API
    if ($use_assistant_id == 'Yes' && $chatbot_ai_platform_choice == 'OpenAI') {
        // DIAG - Diagnostics - Ver 2.3.6
        // back_trace( 'NOTICE', 'Calling OpenAI Assistant API for queued message');
        $response = steven_bot_custom_gpt_call_api($api_key, $message, $assistant_id, $thread_id, $session_id, $user_id, $page_id, $client_message_id);
    } elseif ($use_assistant_id == 'Yes' && $chatbot_ai_platform_choice == 'Azure OpenAI') {
        // DIAG - Diagnostics - Ver 2.3.6
        // back_trace( 'NOTICE', 'Calling Azure OpenAI Assistant API for queued message');
        ...
        // DIAG - Diagnostics - Ver 2.3.6
        // back_trace( 'NOTICE', 'Calling Mistral Agent API for queued message');
        ...
        switch ($chatbot_ai_platform_choice) {
            case 'OpenAI':
                ...
                if (str_starts_with($model, 'gpt-4o')) {
                    $response = steven_bot_call_omni($api_key, $message, $user_id, $page_id, $session_id, $assistant_id, $client_message_id);
                } elseif (str_starts_with($model, 'gpt')) {
                    $response = steven_bot_call_api($api_key, $message, $user_id, $page_id, $session_id, $assistant_id, $client_message_id);
                } elseif (str_starts_with($model, 'dall')) {
                    $response = steven_bot_call_image_api($api_key, $message, $user_id, $page_id, $session_id, $assistant_id, $client_message_id);
                } elseif (str_starts_with($model, 'tts')) {
                    $response = steven_bot_call_tts_api($api_key, $message, $voice, $user_id, $page_id, $session_id, $assistant_id, $client_message_id);
                } elseif (str_starts_with($model, 'whisper')) {
                    $response = steven_bot_call_stt_api($api_key, $message);
                } else {
                    $response = steven_bot_call_api($api_key, $message, $user_id, $page_id, $session_id, $assistant_id, $client_message_id);
                }
                break;
            ...
            case 'Markov Chain':
                $response = steven_bot_call_markov_chain_api($message);
                break;
                
            case 'Transformer':
                $response = steven_bot_call_transformer_model_api($message);
                break;
                
            case 'Local Server':
                $response = steven_bot_call_local_model_api($message);
                break;
                
            case 'Mistral':
                // If use_assistant_id is No, use regular Mistral API
                $response = steven_bot_call_mistral_api($api_key, $message);
                break;
...
function steven_bot_process_single_message($message_data) {
...
function steven_bot_send_message() {
    ...
    */
    ...
    if ($current_user_id === 0) {
        ...
        // For anonymous users, user_id should be 0 (session_id is used in transient key via get_steven_bot_transients)
        $user_id = 0;
    ...
    if ( !$message ) {
        ...
        global $steven_bot_fixed_literal_messages;
        // Define a default fallback message
        $default_message = 'Error: Invalid API key or Message. Please check the plugin settings.';
        $error_message = isset($steven_bot_fixed_literal_messages[15]) 
            ? $steven_bot_fixed_literal_messages[15] 
            : $default_message;
        // Send error response
    ...
    $kchat_settings['thread_id'] = get_steven_bot_transients( 'thread_id', $user_id, $page_id, $session_id);
    $kchat_settings['steven_bot_model'] = get_steven_bot_transients( 'model', $user_id, $page_id, $session_id);
    $kchat_settings['model'] = $kchat_settings['steven_bot_model'];
    $kchat_settings['steven_bot_voice_option'] = get_steven_bot_transients( 'voice', $user_id, $page_id, $session_id);
    $kchat_settings['additional_instructions'] = get_steven_bot_transients( 'additional_instructions', $user_id, $page_id, $session_id);
    $voice = $kchat_settings['steven_bot_voice_option'];
    $model = $kchat_settings['steven_bot_model'];
    ...
    // $kchat_settings = array_merge($kchat_settings, get_steven_bot_threads($user_id, $page_id));
    ...
    $model = isset($kchat_settings['steven_bot_model']) ? $kchat_settings['steven_bot_model'] : '';

    // FIXME - TESTING - Ver 2.1.8
    // back_trace( 'NOTICE', '$model: ' . $model);
    // back_trace( 'NOTICE', '$assistant_id: ' . $assistant_id);

    $voice = isset($kchat_settings['steven_bot_voice_option']) ? $kchat_settings['steven_bot_voice_option'] : '';
    ...
    if ($is_processing) {
        ...
        $enqueued_id = steven_bot_enqueue_message($user_id, $page_id, $session_id, $assistant_id, $message, $client_message_id);
        
        // Return queue status
        global $steven_bot_fixed_literal_messages;
        $default_message = 'Message queued. Processing...';
        $queued_message = isset($steven_bot_fixed_literal_messages[20]) 
            ? $steven_bot_fixed_literal_messages[20] 
            : $default_message;
    ...
    // DIAG - Diagnostics - Ver 2.3.4
    // back_trace( 'NOTICE', 'Set conversation lock - Lock key: ' . $conv_lock);
    ...
    // back_trace( 'NOTICE', '$thread_id: ' . $thread_id);
    // back_trace( 'NOTICE', '$assistant_id: ' . $assistant_id);
    // back_trace( 'NOTICE', '$steven_bot_assistant_alias: ' . $steven_bot_assistant_alias);
    // back_trace( 'NOTICE', '$model: ' . $model);
    ...
    // Assistants
    // $steven_bot_assistant_alias == 'original'; // Default
    // $steven_bot_assistant_alias == 'primary';
    // $steven_bot_assistant_alias == 'alternate';
    // $steven_bot_assistant_alias == 'asst_xxxxxxxxxxxxxxxxxxxxxxxx'; // GPT Assistant Id
    // $steven_bot_assistant_alias == 'ag:xxxxxxxxxxxxxxxxxxxxxxxx'; // MistralAgent Id
    // $steven_bot_assistant_alias == 'websearch'; // Mistral Websearch Id
    ...
    if ($steven_bot_assistant_alias == 'original') {
        ...
        // DIAG - Diagnostics - Ver 2.3.6
        // back_trace( 'NOTICE', 'Using Primary Assistant - $steven_bot_assistant_alias: ' . $steven_bot_assistant_alias);

        $assistant_id = esc_attr(get_option('assistant_id'));
        // $additional_instructions = esc_attr(get_option('steven_bot_assistant_instructions', '')); // REMOVED VER 2.0.9
        $use_assistant_id = 'Yes';
        ...
        // DIAG - Diagnostics - Ver 2.3.6
        // back_trace( 'NOTICE', 'Using Alternate Assistant - $steven_bot_assistant_alias: ' . $steven_bot_assistant_alias);

        $assistant_id = esc_attr(get_option('steven_bot_assistant_id_alternate'));
        // $additional_instructions = esc_attr(get_option('steven_bot_assistant_instructions_alternate', '')); // REMOVED VER 2.0.9
        $use_assistant_id = 'Yes';
    ...
    $additional_instructions = get_steven_bot_transients( 'additional_instructions', $user_id, $page_id, $session_id);

    // Decide whether to use Flow, Assistant or Original ChatGPT
    // DIAG - Diagnostics - Ver 2.3.4
    // back_trace( 'NOTICE', 'Decision point - model: ' . $model . ', use_assistant_id: ' . $use_assistant_id);
    ...
    if ($model == 'flow'){
        ...
        $kflow_step = get_steven_bot_transients( 'kflow_step', null, null, $session_id);
        if (empty($kflow_step)) {
        ...
        // Set the next step
        set_steven_bot_transients( 'kflow_step', $kflow_step, null, null, $session_id);
        ...
        $response = steven_bot_call_flow_api($api_key, $message);
        ...
        // DIAG - Diagnostics - Ver 2.3.4
        // back_trace( 'NOTICE', 'Using Assistant ID path - use_assistant_id: ' . $use_assistant_id);
        // DIAG - Diagnostics - Ver 2.1.1.1
        ...
        // $steven_bot_suppress_learnings = esc_attr(get_option('steven_bot_suppress_learnings', 'Random'));
        // if ( $steven_bot_suppress_learnings != 'None') {
        //     $response = $response . ' ' . steven_bot_enhance_with_tfidf($message);
        // }
        ...
        if (str_starts_with($response, 'Error:') || str_starts_with($response, 'Failed:')) {
            ...
            $error_message = isset($steven_bot_fixed_literal_messages[0]) 
                ? $steven_bot_fixed_literal_messages[0] 
                : $default_message;
            ...
            $response = steven_bot_check_for_links_and_images($response);
        
            // Append any extra message if configured
            $extra_message = esc_attr(get_option('steven_bot_extra_message', ''));
            $response = steven_bot_append_extra_message($response, $extra_message);
        
            // Clear conversation lock and process queue BEFORE sending response
            delete_transient($conv_lock);
            // DIAG - Diagnostics - Ver 2.3.4
            // back_trace( 'NOTICE', 'Cleared conversation lock - Lock key: ' . $conv_lock);
            steven_bot_process_queue($user_id, $page_id, $session_id, $assistant_id);
        ...
        // DIAG - Diagnostics - Ver 2.3.4
        // back_trace( 'NOTICE', 'Using ChatGPT path - use_assistant_id: ' . $use_assistant_id);
        ...
        // If $model starts with 'gpt' then the steven_bot_call_api or 'dall' then steven_bot_call_image_api
        ...
        switch ($chatbot_ai_platform_choice) {
            ...
            default:
                ...
                // Send message to ChatGPT API - Ver 1.6.7
                $response = steven_bot_call_api($api_key, $message);
    ...
    $error_message = isset($steven_bot_fixed_literal_messages[1]) 
        ? $steven_bot_fixed_literal_messages[1] 
        : $default_message;
...
function steven_bot_refresh_nonce() {
...
function steven_bot_clear_context_history() {
...
function steven_bot_clear_stuck_visitor_locks() {
    global $wpdb;
...
add_action('init', 'steven_bot_clear_stuck_visitor_locks', 5);

// Add admin menu for visitor lock clearing tool - Ver 2.3.6
function steven_bot_add_visitor_lock_tool_menu() {
    add_submenu_page(
        'steven-bot',
        'Clear Visitor Locks',
        'Clear Visitor Locks',
        ...
        'steven_bot_visitor_lock_tool_page'
    );
}
add_action('admin_menu', 'steven_bot_add_visitor_lock_tool_menu');

// Admin page for visitor lock clearing tool
function steven_bot_visitor_lock_tool_page() {
    if (isset($_POST['clear_locks']) && wp_verify_nonce($_POST['_wpnonce'], 'clear_visitor_locks')) {
        steven_bot_clear_stuck_visitor_locks();
...
add_action('wp_ajax_steven_bot_send_message', 'steven_bot_send_message');
add_action('wp_ajax_nopriv_steven_bot_send_message', 'steven_bot_send_message');

// Add action to refresh nonce - Ver 2.3.6
add_action('wp_ajax_steven_bot_refresh_nonce', 'steven_bot_refresh_nonce');
add_action('wp_ajax_nopriv_steven_bot_refresh_nonce', 'steven_bot_refresh_nonce');

// Add action to clear context history on page load - Ver 2.3.7
add_action('wp_ajax_steven_bot_clear_context_history', 'steven_bot_clear_context_history');
add_action('wp_ajax_nopriv_steven_bot_clear_context_history', 'steven_bot_clear_context_history');

// Add action to get queue status
add_action('wp_ajax_steven_bot_get_queue_status', 'steven_bot_get_queue_status_ajax');
add_action('wp_ajax_nopriv_steven_bot_get_queue_status', 'steven_bot_get_queue_status_ajax');

// Add action to upload files - Ver 1.7.6 (Security: Authentication required)
add_action('wp_ajax_steven_bot_upload_files', 'steven_bot_upload_files');

// Add action to upload mp3 files - Ver 1.7.6 (Security: Authentication required)
add_action('wp_ajax_steven_bot_upload_mp3', 'steven_bot_upload_mp3');

// Add action to erase conversation - Ver 1.8.6 (Security: Authentication required)
add_action('wp_ajax_steven_bot_erase_conversation', 'steven_bot_erase_conversation_handler');
add_action('wp_ajax_nopriv_steven_bot_erase_conversation', 'steven_bot_erase_conversation_handler');

// Add action to unlock conversation - Ver 2.3.0 (Security: Authentication required)
add_action('wp_ajax_steven_bot_unlock_conversation', 'steven_bot_unlock_conversation_handler');
add_action('wp_ajax_nopriv_steven_bot_unlock_conversation', 'steven_bot_unlock_conversation_handler');

// Add action to reset all locks (Security: Authentication required)
add_action('wp_ajax_steven_bot_reset_all_locks', 'steven_bot_reset_all_locks_handler');

// Add action to reset cache and locks (Security: Authentication required) - Ver 2.3.6
add_action('wp_ajax_steven_bot_reset_cache_locks', 'steven_bot_reset_cache_locks_handler');

// Add action for text-to-speech (Security: Authentication required)
add_action('wp_ajax_steven_bot_read_aloud', 'steven_bot_read_aloud');

// Add action for transcript download (Security: Authentication required)
add_action('wp_ajax_steven_bot_download_transcript', 'steven_bot_download_transcript');

// Settings and Deactivation - Ver 1.5.0
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'steven_bot_plugin_action_links');

// Unlock conversation handler - Ver 2.3.0
function steven_bot_unlock_conversation_handler() {
    ...
    if ($user_id && $page_id && $session_id && $assistant_id) {
        ...
        // DIAG - Diagnostics - Ver 2.3.4
        // back_trace( 'NOTICE', 'Unlock called - Lock key: ' . $conv_lock . ', Exists: ' . ($lock_exists ? 'Yes' : 'No') . ', Deleted: ' . ($deleted_lock ? 'Yes' : 'No'));
        // back_trace( 'NOTICE', 'Unlock called - Queue key: ' . $queue_key . ', Exists: ' . ($queue_exists ? 'Yes' : 'No') . ', Deleted: ' . ($deleted_queue ? 'Yes' : 'No'));
        ...
        foreach ($possible_locks as $lock_key) {
            if (get_transient($lock_key)) {
                ...
                // DIAG - Diagnostics - Ver 2.3.4
                // back_trace( 'NOTICE', 'Cleared additional lock: ' . $lock_key);
            }
        }
        ...
        // DIAG - Diagnostics - Ver 2.3.4
        // back_trace('ERROR', 'Unlock failed - Missing parameters: user_id=' . $user_id . ', page_id=' . $page_id . ', session_id=' . $session_id . ', assistant_id=' . $assistant_id);
        wp_send_json_error('Missing parameters');
    }
}

// Global lock reset handler - Ver 2.3.0
function steven_bot_reset_all_locks_handler() {
    ...
    if ($user_id && $page_id && $session_id && $assistant_id) {
        ...
        foreach ($possible_locks as $lock_key) {
            if (get_transient($lock_key)) {
                ...
                $cleared_count++;
                // DIAG - Diagnostics - Ver 2.3.4
                // back_trace( 'NOTICE', 'Reset cleared lock: ' . $lock_key);
            }
        }
        ...
        foreach ($possible_queues as $queue_key) {
            if (get_transient($queue_key)) {
                ...
                $cleared_count++;
                // DIAG - Diagnostics - Ver 2.3.4
                // back_trace( 'NOTICE', 'Reset cleared queue: ' . $queue_key);
            }
        }
        
        // DIAG - Diagnostics - Ver 2.3.4
        // back_trace( 'NOTICE', 'Reset completed - Cleared ' . $cleared_count . ' locks/queues');
        wp_send_json_success('Reset completed - Cleared ' . $cleared_count . ' locks/queues');

    } else {

        // DIAG - Diagnostics - Ver 2.3.4
        // back_trace('ERROR', 'Reset failed - Missing parameters: user_id=' . $user_id . ', page_id=' . $page_id . ', session_id=' . $session_id . ', assistant_id=' . $assistant_id);
        wp_send_json_error('Missing parameters');
...
function steven_bot_reset_cache_locks_handler() {
    ...
    try {
        ...
        $lock_patterns = [
            ...
            '_transient_steven_bot_%',
            '_transient_timeout_steven_bot_%'
        ];
        ...
        $cache_dirs = [
            ...
            WP_CONTENT_DIR . '/uploads/steven-bot/',
...
function steven_bot_append_extra_message($response, $extra_message) {
...
function steven_bot_kn_status_activation() {

    add_option('steven_bot_kn_status', 'Never Run');
...
register_activation_hook(__FILE__, 'steven_bot_kn_status_activation');

// Clean Up in Aisle 4
function steven_bot_kn_status_deactivation() {

    delete_option('steven_bot_kn_status');
    wp_clear_scheduled_hook('knowledge_navigator_scan_hook'); 

}
register_deactivation_hook(__FILE__, 'steven_bot_kn_status_deactivation');
...
function enqueue_greetings_script( $initial_greeting = null, $subsequent_greeting = null) {
    ...
    if (is_user_logged_in()) {
        ...
        if ( empty($initial_greeting) ) {
            $initial_greeting = esc_attr(get_option('steven_bot_initial_greeting', 'Hello! How can I help you today?'));
        }
        ...
        if ( empty($subsequent_greeting) ) {
            $subsequent_greeting = esc_attr(get_option('steven_bot_subsequent_greeting', 'Hello again! How can I help you?'));
        }
...
function enqueue_color_picker($hook_suffix) {
    ...
    wp_enqueue_script('my-script-handle', plugin_dir_url(__FILE__) . 'assets/js/steven-bot-color-picker.js', array('wp-color-picker'), $steven_bot_plugin_version, true);
...
function kchat_get_plugin_version() {
    ...
    $plugin_data = get_plugin_data(plugin_dir_path(__FILE__) . 'steven-bot.php');
    // DIAG - Print the plugin data
    // back_trace( 'NOTICE', 'Plugin data: ' . print_r($plugin_data, true));
    // $plugin_version = $plugin_data['steven_bot_version'];
